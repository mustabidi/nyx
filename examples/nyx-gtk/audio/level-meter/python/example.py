...
from gi.repository import Adw, Nyx, NyxGtk, Gdk, Gst, Gtk
...
def on_state_changed(player, pspec, bars):
    if (player.props.state < Nyx.PlayerState.PLAYING):
        for bar in bars:
            bar.set_value(0)

def on_activate(app):
    # Create our widgets.
    win = Gtk.ApplicationWindow(application=app, title='Nyx Audio Level Meter', default_width=400, default_height=96)
    audio = NyxGtk.Audio()
...
app = Adw.Application(application_id='com.example.NyxAudioLevelMeter')
