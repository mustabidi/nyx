...
#define NYX_SERVER_MDNS_SERVICE "_nyx._tcp.local"
...
#define GST_CAT_DEFAULT nyx_server_mdns_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

static NyxServerMdns *mdns = NULL;
static GCond mdns_cond;
static GMutex mdns_lock;

void
nyx_server_mdns_debug_init (void)
{
  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxservermdns",
      GST_DEBUG_FG_RED, "Nyx Server MDNS");
}

static void
nyx_server_mdns_entry_free (NyxServerMdnsEntry *entry)
{
...
static void
nyx_server_mdns_remove_port (GPtrArray *entries, guint port)
{
  guint i;

  for (i = 0; i < entries->len; ++i) {
    NyxServerMdnsEntry *entry = g_ptr_array_index (entries, i);
    ...
    enum mdns_announce_type type, GPtrArray *entries)
{
  ...
  for (i = 0; i < entries->len; ++i) {
    ...
    GST_LOG ("Preparing answers for MDNS query, service: \"%s\""
        ", domain: \"%s\", link: \"%s\"",
        NYX_SERVER_MDNS_SERVICE, domain_name, entry->service_link);

    answers[PTR_INDEX(i)] = (struct rr_entry) {
      .type = RR_PTR,
      .name = (char *) NYX_SERVER_MDNS_SERVICE,
...
{
  if (service && strcmp (service, NYX_SERVER_MDNS_SERVICE) != 0)
    return;
...
void
nyx_server_mdns_serve (gchar *name, guint port)
{
  NyxServerMdnsEntry *entry;
  const gchar *prgname = g_get_prgname ();
  gboolean stopped;

  entry = g_new (NyxServerMdnsEntry, 1);
  entry->name = name;
  entry->service_link = g_strdup_printf ("%s %s %s.%s",
      g_get_host_name (), (prgname) ? prgname : "nyxplayer",
      entry->name, NYX_SERVER_MDNS_SERVICE);
  entry->port = port;
  ...
  if ((stopped = mdns == NULL)) {
    mdns = g_new (NyxServerMdns, 1);
    mdns->entries = g_ptr_array_new_with_free_func (
        (GDestroyNotify) nyx_server_mdns_entry_free);
  ...
  if (stopped) {
    ...
    thread = g_thread_try_new ("nyx-server-mdns",
...
void
nyx_server_mdns_remove (guint port)
{
  g_mutex_lock (&mdns_lock);

  nyx_server_mdns_remove_port (mdns->entries, port);
  nyx_server_mdns_remove_port (mdns->pending_entries, port);
