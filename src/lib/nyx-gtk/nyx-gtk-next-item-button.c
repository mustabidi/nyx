...
#include "nyx-gtk-next-item-button.h"
#include "nyx-gtk-utils.h"

#define GST_CAT_DEFAULT nyx_gtk_next_item_button_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxGtkNextItemButton
{
  GtkButton parent;
...
#define parent_class nyx_gtk_next_item_button_parent_class
G_DEFINE_TYPE (NyxGtkNextItemButton, nyx_gtk_next_item_button, GTK_TYPE_BUTTON)

static gboolean
_transform_sensitive_func (GBinding *binding, const GValue *from_value,
    GValue *to_value, NyxGtkNextItemButton *self)
{
  ...
  can_skip = (current_index != NYX_QUEUE_INVALID_POSITION
      && current_index < nyx_queue_get_n_items (queue) - 1);
...
/**
 * nyx_gtk_next_item_button_new:
 *
 * Creates a new #NyxGtkNextItemButton to play next #NyxMediaItem.
 *
 * Returns: a new next item button #GtkWidget.
 */
GtkWidget *
nyx_gtk_next_item_button_new (void)
{
  return g_object_new (NYX_GTK_TYPE_NEXT_ITEM_BUTTON, NULL);
}

static void
nyx_gtk_next_item_button_init (NyxGtkNextItemButton *self)
{
...
static void
nyx_gtk_next_item_button_map (GtkWidget *widget)
{
  NyxGtkNextItemButton *self = NYX_GTK_NEXT_ITEM_BUTTON_CAST (widget);
  NyxPlayer *player;
...
static void
nyx_gtk_next_item_button_unmap (GtkWidget *widget)
{
  NyxGtkNextItemButton *self = NYX_GTK_NEXT_ITEM_BUTTON_CAST (widget);
...
static void
nyx_gtk_next_item_button_class_init (NyxGtkNextItemButtonClass *klass)
{
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxgtknextitembutton", 0,
      "Nyx GTK Next Item Button");

  widget_class->map = nyx_gtk_next_item_button_map;
  widget_class->unmap = nyx_gtk_next_item_button_unmap;
}
