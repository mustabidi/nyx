...
#define GST_CAT_DEFAULT gst_nyx_importer_loader_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

#define parent_class gst_nyx_importer_loader_parent_class
G_DEFINE_TYPE (GstNyxImporterLoader, gst_nyx_importer_loader, GST_TYPE_OBJECT);

typedef GstNyxImporter* (* MakeImporter) (GPtrArray *context_handlers);
...
static void
gst_nyx_importer_data_free (GstNyxImporterData *data)
{
...
_obtain_importer_data (GModule *module, gboolean is_template, GPtrArray *context_handlers)
{
  ...
  data = g_new0 (GstNyxImporterData, 1);
  data->module = module;
  ...
  if (G_UNLIKELY (!data->caps)) {
    ...
    gst_nyx_importer_data_free (data);
...
_obtain_available_modules_once (G_GNUC_UNUSED gpointer data)
{
  ...
  GError *error = NULL;
  const gchar *env_path = g_getenv ("NYX_SINK_IMPORTER_PATH");
  ...
  if (!env_path || env_path[0] == '\0') {
    ...
    dir_path = g_build_filename (win_base_dir,
        "lib", "nyx-0.0", "gst", "plugin", "importers", NULL);
  ...
  if (!dir) {
    const gchar *imp_path = (env_path && env_path[0] != '\0')
        ? env_path : NYX_SINK_IMPORTER_PATH;
...
gst_nyx_importer_loader_get_available_modules (void)
{
...
{
  GstNyxImporterData *data_a, *data_b;

  data_a = *((GstNyxImporterData **) a);
  data_b = *((GstNyxImporterData **) b);
...
_obtain_importers (gboolean is_template, GPtrArray *context_handlers)
{
  ...
  modules = gst_nyx_importer_loader_get_available_modules ();
  importers = g_ptr_array_new_with_free_func (
      (GDestroyNotify) gst_nyx_importer_data_free);
...
GstNyxImporterLoader *
gst_nyx_importer_loader_new (void)
{
  return g_object_new (GST_TYPE_NYX_IMPORTER_LOADER, NULL);
}
...
_make_caps_for_importers (const GPtrArray *importers)
{
  ...
  for (i = 0; i < importers->len; i++) {
    GstNyxImporterData *data = g_ptr_array_index (importers, i);
...
GstPadTemplate *
gst_nyx_importer_loader_make_sink_pad_template (void)
{
  GPtrArray *importers;
  GstCaps *caps;
  ...
  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyximporterloader", 0,
      "Nyx Importer Loader");
...
GstCaps *
gst_nyx_importer_loader_make_actual_caps (GstNyxImporterLoader *self)
{
  return _make_caps_for_importers (self->importers);
}

gboolean
gst_nyx_importer_loader_handle_context_query (GstNyxImporterLoader *self,
    GstBaseSink *bsink, GstQuery *query)
{
  guint i;

  for (i = 0; i < self->context_handlers->len; i++) {
    GstNyxContextHandler *handler = g_ptr_array_index (self->context_handlers, i);

    if (gst_nyx_context_handler_handle_context_query (handler, bsink, query))
      return TRUE;
  }
...
static const GstNyxImporterData *
...
gboolean
gst_nyx_importer_loader_find_importer_for_caps (GstNyxImporterLoader *self,
    GstCaps *caps, GstNyxImporter **importer)
{
  const GstNyxImporterData *data = NULL;
  GstNyxImporter *found_importer = NULL;
  ...
  if (*importer && (self->last_module == data->module)) {
    ...
    gst_nyx_importer_set_caps (*importer, caps);
    goto finish;
  }
  ...
  gst_nyx_importer_set_caps (found_importer, caps);
...
static void
gst_nyx_importer_loader_init (GstNyxImporterLoader *self)
{
...
static void
gst_nyx_importer_loader_finalize (GObject *object)
{
  GstNyxImporterLoader *self = GST_NYX_IMPORTER_LOADER_CAST (object);
...
static void
gst_nyx_importer_loader_class_init (GstNyxImporterLoaderClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;

  gobject_class->finalize = gst_nyx_importer_loader_finalize;
}
