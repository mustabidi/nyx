...
#define WINDOW_CSS_CLASS_NAME       "nyxsinkwindow"
...
#define GST_CAT_DEFAULT gst_nyx_sink_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

static void gst_nyx_sink_navigation_interface_init (
    GstNavigationInterface *iface);

#define parent_class gst_nyx_sink_parent_class
G_DEFINE_TYPE_WITH_CODE (GstNyxSink, gst_nyx_sink, GST_TYPE_VIDEO_SINK,
    G_IMPLEMENT_INTERFACE (GST_TYPE_NAVIGATION,
        gst_nyx_sink_navigation_interface_init));
GST_ELEMENT_REGISTER_DEFINE (nyxsink, "nyxsink", GST_RANK_NONE,
    GST_TYPE_NYX_SINK);

static void
window_clear_no_lock (GstNyxSink *self)
{
  if (!self->window)
    return;
...
static void
widget_clear_no_lock (GstNyxSink *self)
{
  if (!self->widget)
    return;
...
static void
widget_destroy_cb (GtkWidget *widget, GstNyxSink *self)
{
  GST_NYX_SINK_LOCK (self);
  widget_clear_no_lock (self);
  GST_NYX_SINK_UNLOCK (self);
}

static void
window_destroy_cb (GtkWidget *window, GstNyxSink *self)
{
...
static void
calculate_stream_coords (GstNyxSink *self, GtkWidget *widget,
    gdouble x, gdouble y, gdouble *stream_x, gdouble *stream_y)
{
  ...
  gst_gtk_get_width_height_for_rotation (GST_VIDEO_INFO_WIDTH (&self->v_info),
      ...
      gst_nyx_paintable_get_rotation (self->paintable));
...
static void
gst_nyx_sink_widget_motion_event (GtkEventControllerMotion *motion,
    gdouble x, gdouble y, GstNyxSink *self)
{
  GtkWidget *widget;
...
static void
gst_nyx_sink_widget_button_event (GtkGestureClick *click,
    gint n_press, gdouble x, gdouble y, GstNyxSink *self)
{
  GtkWidget *widget;
  GdkEvent *event;
...
static GtkWidget *
gst_nyx_sink_get_widget (GstNyxSink *self)
{
  if (G_UNLIKELY (!self->widget)) {
    ...
    g_signal_connect (controller, "motion",
        G_CALLBACK (gst_nyx_sink_widget_motion_event), self);
    gtk_widget_add_controller (self->widget, controller);

    gesture = gtk_gesture_click_new ();
    gtk_gesture_single_set_button (GTK_GESTURE_SINGLE (gesture), 1);
    g_signal_connect (gesture, "pressed",
        G_CALLBACK (gst_nyx_sink_widget_button_event), self);
    ...
    gst_nyx_paintable_set_widget (self->paintable, self->widget);
...
static GtkWidget *
gst_nyx_sink_obtain_widget (GstNyxSink *self)
{
  GtkWidget *widget;

  GST_NYX_SINK_LOCK (self);
  widget = gst_nyx_sink_get_widget (self);
  if (widget)
    g_object_ref (widget);
  GST_NYX_SINK_UNLOCK (self);
...
static void
gst_nyx_sink_get_property (GObject *object, guint prop_id,
    GValue *value, GParamSpec *pspec)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (object);
  ...
  switch (prop_id) {
    case PROP_WIDGET:
      if (self->widget) {
        ...
        GST_NYX_SINK_UNLOCK (self);
        widget = gst_gtk_invoke_on_main ((GThreadFunc) gst_nyx_sink_obtain_widget, self);
        GST_NYX_SINK_LOCK (self);
...
static void
gst_nyx_sink_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PIXEL_ASPECT_RATIO:
      ...
      gst_nyx_paintable_set_pixel_aspect_ratio (self->paintable,
    ...
    case PROP_ROTATE_METHOD:
      ...
      gst_nyx_paintable_set_rotation (self->paintable,
...
static void
gst_nyx_sink_navigation_send_event (GstNavigation *navigation,
    GstStructure *structure)
{
  GstNyxSink *sink = GST_NYX_SINK_CAST (navigation);
  GstEvent *event;
  GstPad *pad;
...
static gboolean
gst_nyx_sink_propose_allocation (GstBaseSink *bsink, GstQuery *query)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (bsink);
  GstNyxImporter *importer = NULL;
  GstCaps *caps;
  GstVideoInfo info;
  ...
  if (need_pool) {
    ...
    pool = gst_nyx_importer_create_pool (importer, &config);
  ...
  gst_nyx_importer_add_allocation_metas (importer, query);
  gst_object_unref (importer);
...
static gboolean
gst_nyx_sink_query (GstBaseSink *bsink, GstQuery *query)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (bsink);
  gboolean res = FALSE;
  ...
  if (GST_QUERY_TYPE (query) == GST_QUERY_CONTEXT) {
    GST_NYX_SINK_LOCK (self);
    res = gst_nyx_importer_loader_handle_context_query (self->loader, bsink, query);
    GST_NYX_SINK_UNLOCK (self);
  }
...
static gboolean
gst_nyx_sink_start_on_main (GstNyxSink *self)
{
  GtkWidget *widget;
  ...
  if (!(widget = gst_nyx_sink_get_widget (self))) {
    GST_NYX_SINK_UNLOCK (self);
  ...
  if (G_UNLIKELY (!gtk_widget_get_root (widget) && !self->window)) {
    ...
    win_title = g_strdup_printf ("Nyx Sink - GTK %u.%u.%u Window",
...
static gboolean
gst_nyx_sink_start (GstBaseSink *bsink)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (bsink);
  gboolean with_nyx_gtk;

  GST_INFO_OBJECT (self, "Start");

  with_nyx_gtk = g_type_from_name ("NyxGtkVideo");

  if (G_UNLIKELY (!with_nyx_gtk && !(! !gst_gtk_invoke_on_main (
      (GThreadFunc) (GCallback) gst_nyx_sink_start_on_main, self)))) {
...
static gboolean
gst_nyx_sink_stop_on_main (GstNyxSink *self)
{
...
static gboolean
gst_nyx_sink_stop (GstBaseSink *bsink)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (bsink);
  gboolean has_window;
  ...
  if (G_UNLIKELY (has_window)) {
    return (! !gst_gtk_invoke_on_main ((GThreadFunc) (GCallback)
        gst_nyx_sink_stop_on_main, self));
  }
...
static gboolean
gst_nyx_sink_event (GstBaseSink *bsink, GstEvent *event)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (bsink);
  GstTagList *taglist;
    ...
    case GST_EVENT_TAG:
      ...
      if (gst_video_orientation_from_tag (taglist, &orientation)) {
        ...
        if (self->rotation_mode == GST_VIDEO_ORIENTATION_AUTO)
          gst_nyx_paintable_set_rotation (self->paintable, orientation);
        GST_NYX_SINK_UNLOCK (self);
      }
      break;
    default:
...
gst_nyx_sink_change_state (GstElement *element, GstStateChange transition)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (element);
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      ...
      if (self->rotation_mode == GST_VIDEO_ORIENTATION_AUTO)
        gst_nyx_paintable_set_rotation (self->paintable, self->stream_orientation);
      GST_NYX_SINK_UNLOCK (self);
      break;
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      GST_NYX_SINK_LOCK (self);
      if (!self->keep_last_frame && self->importer) {
        gst_nyx_importer_set_buffer (self->importer, NULL);
        gst_nyx_paintable_queue_draw (self->paintable);
      }
      GST_NYX_SINK_UNLOCK (self);
      break;
    default:
...
static void
gst_nyx_sink_get_times (GstBaseSink *bsink, GstBuffer *buffer,
...
static GstCaps *
gst_nyx_sink_get_caps (GstBaseSink *bsink, GstCaps *filter)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (bsink);
  GstCaps *result, *tmp;

  tmp = gst_nyx_importer_loader_make_actual_caps (self->loader);
...
static gboolean
gst_nyx_sink_set_caps (GstBaseSink *bsink, GstCaps *caps)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (bsink);
  ...
  if (!gst_nyx_importer_loader_find_importer_for_caps (self->loader, caps, &self->importer)) {
    GST_NYX_SINK_UNLOCK (self);
    GST_ELEMENT_ERROR (self, RESOURCE, NOT_FOUND,
  ...
  gst_nyx_paintable_set_importer (self->paintable, self->importer);
...
static gboolean
gst_nyx_sink_set_info (GstVideoSink *vsink, GstCaps *caps, const GstVideoInfo *info)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (vsink);
  gboolean res;
  ...
  res = gst_nyx_paintable_set_video_info (self->paintable, info);
  GST_NYX_SINK_UNLOCK (self);
...
gst_nyx_sink_show_frame (GstVideoSink *vsink, GstBuffer *buffer)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (vsink);
  ...
  gst_nyx_importer_set_buffer (self->importer, buffer);
  gst_nyx_paintable_queue_draw (self->paintable);
...
static void
gst_nyx_sink_init (GstNyxSink *self)
{
  GObjectClass *gobject_class;

  gobject_class = (GObjectClass *) GST_NYX_SINK_GET_CLASS (self);
  ...
  self->paintable = gst_nyx_paintable_new ();
  self->loader = gst_nyx_importer_loader_new ();
}

static void
gst_nyx_sink_dispose (GObject *object)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (object);
...
static void
gst_nyx_sink_finalize (GObject *object)
{
  GstNyxSink *self = GST_NYX_SINK_CAST (object);
...
static void
gst_nyx_sink_class_init (GstNyxSinkClass *klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxsink", 0,
      "Nyx Sink");

  gobject_class->get_property = gst_nyx_sink_get_property;
  gobject_class->set_property = gst_nyx_sink_set_property;
  gobject_class->dispose = gst_nyx_sink_dispose;
  gobject_class->finalize = gst_nyx_sink_finalize;
  ...
  gstelement_class->change_state = gst_nyx_sink_change_state;

  gstbasesink_class->get_caps = gst_nyx_sink_get_caps;
  gstbasesink_class->set_caps = gst_nyx_sink_set_caps;
  gstbasesink_class->get_times = gst_nyx_sink_get_times;
  gstbasesink_class->propose_allocation = gst_nyx_sink_propose_allocation;
  gstbasesink_class->query = gst_nyx_sink_query;
  gstbasesink_class->start = gst_nyx_sink_start;
  gstbasesink_class->stop = gst_nyx_sink_stop;
  gstbasesink_class->event = gst_nyx_sink_event;

  gstvideosink_class->set_info = gst_nyx_sink_set_info;
  gstvideosink_class->show_frame = gst_nyx_sink_show_frame;

  gst_element_class_set_static_metadata (gstelement_class,
      "Nyx video sink",
      "Sink/Video", "A GTK4 video sink used by Nyx media player",
      "Rafał Dzięgiel <rafostar.github@gmail.com>");

  sink_pad_templ = gst_nyx_importer_loader_make_sink_pad_template ();
 ...
 */
static void
gst_nyx_sink_navigation_interface_init (GstNavigationInterface *iface)
{
  /* TODO: Port to "send_event_simple" once we depend on GStreamer 1.22 */
  iface->send_event = gst_nyx_sink_navigation_send_event;
}
