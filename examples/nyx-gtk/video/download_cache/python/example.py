...
from gi.repository import Adw, Nyx, NyxGtk, GLib, Gtk
import shutil
...
def on_activate(app):
    ...
    video = NyxGtk.Video()
    controls = NyxGtk.SimpleControls(fullscreenable=False)
    ...
    # Configure playback
    video.props.player.props.queue.set_progression_mode(Nyx.QueueProgressionMode.CAROUSEL)
    ...
    item_1 = Nyx.MediaItem(uri='http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4')
    item_2 = Nyx.MediaItem(uri='http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4')
    video.props.player.props.queue.add_item(item_1)
    video.props.player.props.queue.add_item(item_2)

# Create a new application
app = Adw.Application(application_id='com.example.NyxDownloadCache')
