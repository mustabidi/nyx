...
#define GST_CAT_DEFAULT nyx_app_queue_list_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxAppQueueList
{
  GtkBox parent;
...
#define parent_class nyx_app_queue_list_parent_class
G_DEFINE_TYPE (NyxAppQueueList, nyx_app_queue_list, GTK_TYPE_BOX);
...
typedef struct
{
  ...
  gdouble x, y;
} NyxAppQueueListDragData;

static GdkContentProvider *
drag_item_prepare_cb (GtkDragSource *drag_source, gdouble x, gdouble y, NyxAppQueueList *self)
{
  GtkWidget *list_view, *pickup, *list_widget;
  GdkPaintable *paintable;
  NyxMediaItem *item;
  NyxAppQueueListDragData *drag_data;
  graphene_point_t p;
  ...
  if (G_UNLIKELY (pickup == NULL) || !NYX_APP_IS_MEDIA_ITEM_BOX (pickup))
    return NULL;

  list_widget = gtk_widget_get_parent (pickup);
  item = nyx_app_media_item_box_get_media_item (NYX_APP_MEDIA_ITEM_BOX_CAST (pickup));
  ...
  drag_data = g_new (NyxAppQueueListDragData, 1);
...
static void
drag_item_drag_begin_cb (GtkDragSource *drag_source, GdkDrag *drag, NyxAppQueueList *self)
{
  NyxAppQueueListDragData *drag_data;
  GtkWidget *list_view;

  drag_data = (NyxAppQueueListDragData *) g_object_get_data (G_OBJECT (self), "drag-data");
...
drag_item_drag_end_cb (GtkDragSource *drag_source, GdkDrag *drag,
    gboolean delete_data, NyxAppQueueList *self)
{
  NyxAppQueueListDragData *drag_data;
  GtkWidget *list_view;
...
queue_drop_value_notify_cb (GtkDropTarget *drop_target,
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppQueueList *self)
{
...
queue_drop_motion_cb (GtkDropTarget *drop_target,
    gdouble x, gdouble y, NyxAppQueueList *self)
{
  ...
  if (pickup && NYX_APP_IS_MEDIA_ITEM_BOX (pickup)) {
    NyxAppQueueListDragData *drag_data;
...
static void
queue_drop_leave_cb (GtkDropTarget *drop_target, NyxAppQueueList *self)
{
  if (self->list_target) {
    gtk_widget_set_margin_top (self->list_target, 0);
    gtk_widget_set_margin_bottom (self->list_target, 0);
...
queue_drop_cb (GtkDropTarget *drop_target, const GValue *value,
    gdouble x, gdouble y, NyxAppQueueList *self)
{
  NyxPlayer *player = nyx_gtk_get_player_from_ancestor (GTK_WIDGET (self));
  ...
  if (self->list_target) {
    ...
    if (!nyx_queue_find_item (queue, item, &drop_index))
      return FALSE;

    if (self->drop_after)
      drop_index++;
  } else {
    drop_index = nyx_queue_get_n_items (queue); // Append
  }
  ...
  if (G_VALUE_HOLDS (value, GTK_TYPE_WIDGET)) {
    ...
    if (drag_data) {
      ...
      if (item != drag_data->item) {
        ...
        if (nyx_queue_find_item (queue, drag_data->item, &index)) {
          if (drop_index > index)
            drop_index--;

          nyx_queue_reposition_item (queue, drag_data->item, drop_index);
          success = TRUE;
        }
      }
      ...
      if (G_LIKELY (NYX_APP_IS_MEDIA_ITEM_BOX (drop_widget))) {
        NyxMediaItem *drop_item;
        NyxQueue *src_queue;

        drop_item = nyx_app_media_item_box_get_media_item (
            NYX_APP_MEDIA_ITEM_BOX_CAST (drop_widget));

        if ((src_queue = NYX_QUEUE (gst_object_get_parent (GST_OBJECT (drop_item))))) {
          gst_object_ref (drop_item); // Ref so it survives queue removal

          nyx_queue_remove_item (src_queue, drop_item);
          nyx_queue_insert_item (queue, drop_item, drop_index);
          success = TRUE;
    ...
    if (nyx_app_utils_files_from_value (value, &files, &n_files)) {
      ...
      for (i = 0; i < n_files; ++i) {
        NyxMediaItem *new_item = nyx_media_item_new_from_file (files[i]);

        nyx_queue_insert_item (queue, new_item, drop_index + i);
    ...
    gdouble x, gdouble y, NyxAppQueueList *self)
{
  ...
  if ((queue = NYX_QUEUE (gst_object_get_parent (GST_OBJECT (drag_data->item))))) {
    nyx_queue_remove_item (queue, drag_data->item);
...
static void
_item_selected_cb (NyxAppQueueSelection *selection, guint index, NyxAppQueueList *self)
{
...
_queue_progression_mode_transform_to_func (GBinding *binding, const GValue *from_value,
    GValue *to_value, NyxAppQueueList *self)
{
  NyxQueueProgressionMode mode = g_value_get_enum (from_value);
    ...
    GValue *to_value, NyxAppQueueList *self)
{
  ...
  g_value_set_enum (to_value, (NyxQueueProgressionMode) mode);
  return TRUE;
}

static void
nyx_app_queue_list_realize (GtkWidget *widget)
{
  NyxAppQueueList *self = NYX_APP_QUEUE_LIST_CAST (widget);
  NyxPlayer *player;
  ...
  if ((player = nyx_gtk_get_player_from_ancestor (widget))) {
    NyxQueue *queue = nyx_player_get_queue (player);
    NyxAppQueueSelection *selection = nyx_app_queue_selection_new (queue);
...
static void
nyx_app_queue_list_unrealize (GtkWidget *widget)
{
  NyxAppQueueList *self = NYX_APP_QUEUE_LIST_CAST (widget);
...
static void
nyx_app_queue_list_init (NyxAppQueueList *self)
{
...
static void
nyx_app_queue_list_dispose (GObject *object)
{
  gtk_widget_dispose_template (GTK_WIDGET (object), NYX_APP_TYPE_QUEUE_LIST);

  G_OBJECT_CLASS (parent_class)->dispose (object);
}

static void
nyx_app_queue_list_finalize (GObject *object)
{
  NyxAppQueueList *self = NYX_APP_QUEUE_LIST_CAST (object);
...
static void
nyx_app_queue_list_class_init (NyxAppQueueListClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxappqueuelist", 0,
      "Nyx App Queue List");

  gobject_class->dispose = nyx_app_queue_list_dispose;
  gobject_class->finalize = nyx_app_queue_list_finalize;

  widget_class->realize = nyx_app_queue_list_realize;
  widget_class->unrealize = nyx_app_queue_list_unrealize;

  gtk_widget_class_set_template_from_resource (widget_class,
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-queue-list.ui");

  gtk_widget_class_bind_template_child (widget_class, NyxAppQueueList, progression_drop_down);
  gtk_widget_class_bind_template_child (widget_class, NyxAppQueueList, list_view);

  gtk_widget_class_bind_template_child (widget_class, NyxAppQueueList, stack);
  gtk_widget_class_bind_template_child (widget_class, NyxAppQueueList, stack_default_page);
  gtk_widget_class_bind_template_child (widget_class, NyxAppQueueList, stack_trash_page);

  gtk_widget_class_bind_template_child (widget_class, NyxAppQueueList, trash_drop_target);
  gtk_widget_class_bind_template_child (widget_class, NyxAppQueueList, drop_target);
  ...
  gtk_widget_class_set_css_name (widget_class, "nyx-app-queue-list");
}
