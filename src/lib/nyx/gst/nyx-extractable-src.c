...
static GstURIType
nyx_extractable_src_uri_handler_get_type (GType type)
{
  return GST_URI_SRC;
}
...
_make_schemes (gpointer user_data G_GNUC_UNUSED)
{
  ...
  gchar **schemes_strv;
  guint i, n_schemes, n_proxies = nyx_enhancer_proxy_list_get_n_proxies (proxies);
  ...
  for (i = 0; i < n_proxies; ++i) {
    ...
    if (nyx_enhancer_proxy_target_has_interface (proxy, NYX_TYPE_EXTRACTABLE)
        && (schemes = nyx_enhancer_proxy_get_extra_data (proxy, "X-Schemes"))) {
      gchar **tmp_strv;
      gint j;
 ...
 */
static gboolean
_extractable_check_for_uri (NyxExtractableSrc *self, GUri *uri)
{
  NyxEnhancerProxyList *proxies = nyx_get_global_enhancer_proxies ();
  gboolean is_https;
  guint i, n_proxies;
 ...
 * @self: a #NyxExtractableSrc
 * @proxies: a #NyxEnhancerProxyList
 * @uri: a #GUri
 *
 ...
 */
static GList *
_filter_extractables_for_uri (NyxExtractableSrc *self,
    NyxEnhancerProxyList *proxies, GUri *uri)
{
  GList *sublist = NULL;
  guint i, n_proxies;
  gboolean is_https;
...
nyx_extractable_src_uri_handler_get_protocols (GType type)
{
...
static gchar *
nyx_extractable_src_uri_handler_get_uri (GstURIHandler *handler)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (handler);
  gchar *uri;
...
static gboolean
nyx_extractable_src_uri_handler_set_uri (GstURIHandler *handler,
...
{
  iface->get_type = nyx_extractable_src_uri_handler_get_type;
  iface->get_protocols = nyx_extractable_src_uri_handler_get_protocols;
  iface->get_uri = nyx_extractable_src_uri_handler_get_uri;
  iface->set_uri = nyx_extractable_src_uri_handler_set_uri;
}

#define parent_class nyx_extractable_src_parent_class
G_DEFINE_TYPE_WITH_CODE (NyxExtractableSrc, nyx_extractable_src, GST_TYPE_PUSH_SRC,
    G_IMPLEMENT_INTERFACE (GST_TYPE_URI_HANDLER, _uri_handler_iface_init));
GST_ELEMENT_REGISTER_DEFINE (nyxextractablesrc, "nyxextractablesrc",
    512, NYX_TYPE_EXTRACTABLE_SRC);

static gboolean
nyx_extractable_src_start (GstBaseSrc *base_src)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (base_src);
  gboolean can_start;
...
static gboolean
nyx_extractable_src_stop (GstBaseSrc *base_src)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (base_src);
...
static gboolean
nyx_extractable_src_get_size (GstBaseSrc *base_src, guint64 *size)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (base_src);
...
static gboolean
nyx_extractable_src_is_seekable (GstBaseSrc *base_src)
{
  return FALSE;
}

static gboolean
nyx_extractable_src_unlock (GstBaseSrc *base_src)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (base_src);
...
static gboolean
nyx_extractable_src_unlock_stop (GstBaseSrc *base_src)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (base_src);
...
static void
_push_events (NyxExtractableSrc *self, GstTagList *tags, GstToc *toc,
...
nyx_extractable_src_create (GstPushSrc *push_src, GstBuffer **outbuf)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (push_src);
  ...
  harvest = nyx_enhancer_director_extract (self->director,
  ...
  unpacked = nyx_harvest_unpack (harvest, outbuf, &self->buf_size,
...
static gboolean
nyx_extractable_src_query (GstBaseSrc *base_src, GstQuery *query)
{
  gboolean ret = FALSE;
...
static void
nyx_extractable_src_set_enhancer_proxies (NyxExtractableSrc *self,
    NyxEnhancerProxyList *enhancer_proxies)
{
  GST_OBJECT_LOCK (self);
...
static void
nyx_extractable_src_init (NyxExtractableSrc *self)
{
  self->cancellable = g_cancellable_new ();
}

static void
nyx_extractable_src_dispose (GObject *object)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (object);
...
static void
nyx_extractable_src_finalize (GObject *object)
{
  NyxExtractableSrc *self = NYX_EXTRACTABLE_SRC_CAST (object);
...
static void
nyx_extractable_src_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ENHANCER_PROXIES:
      nyx_extractable_src_set_enhancer_proxies (self, g_value_get_object (value));
      break;
    default:
...
static void
nyx_extractable_src_get_property (GObject *object, guint prop_id,
...
static void
nyx_extractable_src_class_init (NyxExtractableSrcClass *klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxextractablesrc", 0,
      "Nyx Extractable Source");

  gobject_class->set_property = nyx_extractable_src_set_property;
  gobject_class->get_property = nyx_extractable_src_get_property;
  gobject_class->dispose = nyx_extractable_src_dispose;
  gobject_class->finalize = nyx_extractable_src_finalize;

  gstbasesrc_class->start = nyx_extractable_src_start;
  gstbasesrc_class->stop = nyx_extractable_src_stop;
  gstbasesrc_class->get_size = nyx_extractable_src_get_size;
  gstbasesrc_class->is_seekable = nyx_extractable_src_is_seekable;
  gstbasesrc_class->unlock = nyx_extractable_src_unlock;
  gstbasesrc_class->unlock_stop = nyx_extractable_src_unlock_stop;
  gstbasesrc_class->query = nyx_extractable_src_query;

  gstpushsrc_class->create = nyx_extractable_src_create;
  ...
  gst_element_class_set_static_metadata (gstelement_class, "Nyx Extractable Source",
      "Source", "A source element that uses Nyx extractable enhancers to produce data",
