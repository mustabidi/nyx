...
#define GST_CAT_DEFAULT nyx_gtk_seek_bar_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxGtkSeekBar
{
  GtkWidget parent;
...
static void
nyx_gtk_seek_bar_add_child (GtkBuildable *buildable,
...
{
  iface->add_child = nyx_gtk_seek_bar_add_child;
}

#define parent_class nyx_gtk_seek_bar_parent_class
G_DEFINE_TYPE_WITH_CODE (NyxGtkSeekBar, nyx_gtk_seek_bar, GTK_TYPE_WIDGET,
...
_prepare_popover (NyxGtkSeekBar *self, gdouble x,
    gdouble pointing_val, gdouble upper)
{
  ...
  if (!found_title) {
    NyxTimeline *timeline = nyx_media_item_get_timeline (self->current_item);
    guint i = nyx_timeline_get_n_markers (timeline);
    ...
    while (i--) {
      ...
      self->curr_marker_start = nyx_marker_get_start (marker);

      if (self->curr_marker_start <= pointing_val) {
        const gchar *title = nyx_marker_get_title (marker);
...
static inline gboolean
_compute_scale_coords (NyxGtkSeekBar *self,
...
static void
_player_position_changed_cb (NyxPlayer *player,
    GParamSpec *pspec G_GNUC_UNUSED, NyxGtkSeekBar *self)
{
...
static void
_player_seek_done_cb (NyxPlayer *player, NyxGtkSeekBar *self)
{
  GST_DEBUG ("Seek done");
...
static void
scale_value_changed_cb (GtkRange *range, NyxGtkSeekBar *self)
{
  gdouble value = gtk_range_get_value (range);
  gchar *position_str = g_strdup_printf ("%" NYX_TIME_FORMAT, NYX_TIME_ARGS (value));
    ...
    GParamSpec *pspec G_GNUC_UNUSED, NyxGtkSeekBar *self)
{
  ...
  if (self->has_markers
    ...
    nyx_player_seek_custom (self->player, value,
        MIN (self->seek_method, NYX_PLAYER_SEEK_METHOD_NORMAL));
  } else {
    nyx_player_seek_custom (self->player, value, self->seek_method);
  }
}

static void
scale_scroll_begin_cb (GtkEventControllerScroll *scroll, NyxGtkSeekBar *self)
{
  self->can_scrub = TRUE;
}
...
scale_scroll_cb (GtkEventControllerScroll *scroll,
    gdouble dx, gdouble dy, NyxGtkSeekBar *self)
{
...
static void
scale_scroll_end_cb (GtkEventControllerScroll *scroll, NyxGtkSeekBar *self)
{
  if (self->scrubbing) {
...
motion_cb (GtkEventControllerMotion *motion,
    gdouble x, gdouble y, NyxGtkSeekBar *self)
{
...
static void
motion_leave_cb (GtkEventControllerMotion *motion, NyxGtkSeekBar *self)
{
  gtk_popover_popdown (self->popover);
}
...
static void
_update_duration_label (NyxGtkSeekBar *self, gdouble duration)
{
  GtkAdjustment *adjustment = gtk_range_get_adjustment (GTK_RANGE (self->scale));
  gchar *duration_str = g_strdup_printf ("%" NYX_TIME_FORMAT, NYX_TIME_ARGS (duration));
...
static void
_update_scale_marks (NyxGtkSeekBar *self, NyxTimeline *timeline)
{
  GtkAdjustment *adjustment;
  guint i, n_markers = nyx_timeline_get_n_markers (timeline);
  ...
  for (i = 0; i < n_markers; ++i) {
    ...
    gdouble start = nyx_marker_get_start (marker);
    ...
    if (marker_type >= NYX_MARKER_TYPE_CUSTOM_1) {
      ...
      if (_find_marks_in_widget (self->scale, &top_marks, &bottom_marks)
        ...
        switch (marker_type) {
          case NYX_MARKER_TYPE_CUSTOM_1:
            custom_name = "custom1";
            break;
          case NYX_MARKER_TYPE_CUSTOM_2:
            custom_name = "custom2";
            break;
          case NYX_MARKER_TYPE_CUSTOM_3:
            custom_name = "custom3";
            break;
          default:
            custom_name = NULL;
...
static void
_current_item_duration_changed_cb (NyxMediaItem *current_item,
    GParamSpec *pspec G_GNUC_UNUSED, NyxGtkSeekBar *self)
{
  ...
  _update_duration_label (self, nyx_media_item_get_duration (current_item));
  _update_scale_marks (self, nyx_media_item_get_timeline (current_item));
}

static void
_timeline_markers_changed_cb (GListModel *list_model, guint position,
    guint removed, guint added, NyxGtkSeekBar *self)
{
  _update_scale_marks (self, NYX_TIMELINE (list_model));
}
    ...
    GParamSpec *pspec G_GNUC_UNUSED, NyxGtkSeekBar *self)
{
  ...
  if (self->current_item) {
    ...
    _update_duration_label (self, nyx_media_item_get_duration (self->current_item));
...
/**
 * nyx_gtk_seek_bar_new:
 *
 * Creates a new #NyxGtkSeekBar instance.
 *
 * Returns: a new seek bar #GtkWidget.
 */
GtkWidget *
nyx_gtk_seek_bar_new (void)
{
  return g_object_new (NYX_GTK_TYPE_SEEK_BAR, NULL);
}

/**
 * nyx_gtk_seek_bar_set_reveal_labels:
 * @seek_bar: a #NyxGtkSeekBar
 * @reveal: whether to reveal labels
 *
 * Set whether the position and duration labels should be revealed.
 */
void
nyx_gtk_seek_bar_set_reveal_labels (NyxGtkSeekBar *self, gboolean reveal)
{
  g_return_if_fail (NYX_GTK_IS_SEEK_BAR (self));
...
/**
 * nyx_gtk_seek_bar_get_reveal_labels:
 * @seek_bar: a #NyxGtkSeekBar
 *
 ...
 */
gboolean
nyx_gtk_seek_bar_get_reveal_labels (NyxGtkSeekBar *self)
{
  g_return_val_if_fail (NYX_GTK_IS_SEEK_BAR (self), FALSE);
 ...
 *
 * Set [enum@Nyx.PlayerSeekMethod] to use when seeking with seek bar.
 */
void
nyx_gtk_seek_bar_set_seek_method (NyxGtkSeekBar *self, NyxPlayerSeekMethod method)
{
  g_return_if_fail (NYX_GTK_IS_SEEK_BAR (self));
...
/**
 * nyx_gtk_seek_bar_get_seek_method:
 * @seek_bar: a #NyxGtkSeekBar
 *
 * Get [enum@Nyx.PlayerSeekMethod] used when seeking with seek bar.
 *
 * Returns: #NyxPlayerSeekMethod used for seeking.
 */
NyxPlayerSeekMethod
nyx_gtk_seek_bar_get_seek_method (NyxGtkSeekBar *self)
{
  g_return_val_if_fail (NYX_GTK_IS_SEEK_BAR (self), DEFAULT_SEEK_METHOD);

  return self->seek_method;
}

static void
nyx_gtk_seek_bar_init (NyxGtkSeekBar *self)
{
...
static void
nyx_gtk_seek_bar_compute_expand (GtkWidget *widget,
...
static void
nyx_gtk_seek_bar_size_allocate (GtkWidget *widget,
    gint width, gint height, gint baseline)
{
  NyxGtkSeekBar *self = NYX_GTK_SEEK_BAR_CAST (widget);
...
static void
nyx_gtk_seek_bar_realize (GtkWidget *widget)
{
  NyxGtkSeekBar *self = NYX_GTK_SEEK_BAR_CAST (widget);

  if ((self->player = nyx_gtk_get_player_from_ancestor (widget))) {
    NyxQueue *queue = nyx_player_get_queue (self->player);
...
static void
nyx_gtk_seek_bar_unrealize (GtkWidget *widget)
{
  NyxGtkSeekBar *self = NYX_GTK_SEEK_BAR_CAST (widget);
...
static void
nyx_gtk_seek_bar_map (GtkWidget *widget)
{
  NyxGtkSeekBar *self = NYX_GTK_SEEK_BAR_CAST (widget);
...
static void
nyx_gtk_seek_bar_unmap (GtkWidget *widget)
{
  NyxGtkSeekBar *self = NYX_GTK_SEEK_BAR_CAST (widget);
...
static void
nyx_gtk_seek_bar_dispose (GObject *object)
{
  NyxGtkSeekBar *self = NYX_GTK_SEEK_BAR_CAST (object);

  gtk_widget_dispose_template (GTK_WIDGET (object), NYX_GTK_TYPE_SEEK_BAR);
...
static void
nyx_gtk_seek_bar_finalize (GObject *object)
{
  NyxGtkSeekBar *self = NYX_GTK_SEEK_BAR_CAST (object);
...
static void
nyx_gtk_seek_bar_get_property (GObject *object, guint prop_id,
    GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_REVEAL_LABELS:
      g_value_set_boolean (value, nyx_gtk_seek_bar_get_reveal_labels (self));
      break;
    case PROP_SEEK_METHOD:
      g_value_set_enum (value, nyx_gtk_seek_bar_get_seek_method (self));
      break;
    default:
...
static void
nyx_gtk_seek_bar_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_REVEAL_LABELS:
      nyx_gtk_seek_bar_set_reveal_labels (self, g_value_get_boolean (value));
      break;
    case PROP_SEEK_METHOD:
      nyx_gtk_seek_bar_set_seek_method (self, g_value_get_enum (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static void
nyx_gtk_seek_bar_class_init (NyxGtkSeekBarClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxgtkseekbar", 0,
      "Nyx GTK Seek Bar");

  gobject_class->get_property = nyx_gtk_seek_bar_get_property;
  gobject_class->set_property = nyx_gtk_seek_bar_set_property;
  gobject_class->dispose = nyx_gtk_seek_bar_dispose;
  gobject_class->finalize = nyx_gtk_seek_bar_finalize;

  widget_class->compute_expand = nyx_gtk_seek_bar_compute_expand;
  widget_class->size_allocate = nyx_gtk_seek_bar_size_allocate;
  widget_class->realize = nyx_gtk_seek_bar_realize;
  widget_class->unrealize = nyx_gtk_seek_bar_unrealize;
  widget_class->map = nyx_gtk_seek_bar_map;
  widget_class->unmap = nyx_gtk_seek_bar_unmap;

  /**
   * NyxGtkSeekBar:reveal-labels:
   *
  ...
  gtk_widget_class_set_template_from_resource (widget_class,
      NYX_GTK_RESOURCE_PREFIX "/ui/nyx-gtk-seek-bar.ui");

  gtk_widget_class_bind_template_child (widget_class, NyxGtkSeekBar, position_revealer);
  gtk_widget_class_bind_template_child (widget_class, NyxGtkSeekBar, position_label);
  gtk_widget_class_bind_template_child (widget_class, NyxGtkSeekBar, scale);
  gtk_widget_class_bind_template_child (widget_class, NyxGtkSeekBar, popover);
  gtk_widget_class_bind_template_child (widget_class, NyxGtkSeekBar, popover_label);
  gtk_widget_class_bind_template_child (widget_class, NyxGtkSeekBar, duration_revealer);
  gtk_widget_class_bind_template_child (widget_class, NyxGtkSeekBar, duration_label);
  ...
  gtk_widget_class_set_css_name (widget_class, "nyx-gtk-seek-bar");
}
