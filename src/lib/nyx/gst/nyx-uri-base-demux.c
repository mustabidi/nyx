...
#define GST_CAT_DEFAULT nyx_uri_base_demux_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

typedef struct _NyxUriBaseDemuxPrivate NyxUriBaseDemuxPrivate;

struct _NyxUriBaseDemuxPrivate
{
...
#define parent_class nyx_uri_base_demux_parent_class
G_DEFINE_TYPE_WITH_PRIVATE (NyxUriBaseDemux, nyx_uri_base_demux, GST_TYPE_BIN);

static gboolean
remove_sometimes_pad_cb (GstElement *element, GstPad *pad, NyxUriBaseDemux *self)
{
...
static void
nyx_uri_base_demux_reset (NyxUriBaseDemux *self)
{
  NyxUriBaseDemuxPrivate *priv = nyx_uri_base_demux_get_instance_private (self);
...
nyx_uri_base_demux_change_state (GstElement *element, GstStateChange transition)
{
  NyxUriBaseDemux *self = NYX_URI_BASE_DEMUX_CAST (element);
...
static gboolean
_feature_filter (GstPluginFeature *feature, NyxUriBaseDemuxFilterData *filter_data)
{
...
static GstElement *
_make_handler_for_uri (NyxUriBaseDemux *self, const gchar *uri, const gchar *blacklisted_el)
{
  GstElement *element = NULL;
  GList *factories, *f;
  NyxUriBaseDemuxFilterData filter_data;
  gchar *protocol;
...
_src_pad_query_func (GstPad *pad, GstObject *parent, GstQuery *query)
{
  if (GST_QUERY_TYPE (query) == GST_QUERY_CUSTOM) {
    NyxUriBaseDemux *self = NYX_URI_BASE_DEMUX_CAST (parent);
    NyxUriBaseDemuxClass *uri_bd_class = NYX_URI_BASE_DEMUX_GET_CLASS (self);
...
gboolean
nyx_uri_base_demux_set_uri (NyxUriBaseDemux *self, const gchar *uri, const gchar *blacklisted_el)
{
  NyxUriBaseDemuxPrivate *priv = nyx_uri_base_demux_get_instance_private (self);
...
static gboolean
nyx_uri_base_demux_sink_event (GstPad *pad, GstObject *parent, GstEvent *event)
{
  NyxUriBaseDemux *self = NYX_URI_BASE_DEMUX_CAST (parent);
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_EOS:{
      ...
      success = NYX_URI_BASE_DEMUX_GET_CLASS (self)->process_buffer (self, buffer, cancellable);
...
nyx_uri_base_demux_sink_chain (GstPad *pad, GstObject *parent, GstBuffer *buffer)
{
  NyxUriBaseDemux *self = NYX_URI_BASE_DEMUX_CAST (parent);
  NyxUriBaseDemuxPrivate *priv = nyx_uri_base_demux_get_instance_private (self);
...
static void
nyx_uri_base_demux_init (NyxUriBaseDemux *self)
{
  NyxUriBaseDemuxPrivate *priv = nyx_uri_base_demux_get_instance_private (self);
...
static void
nyx_uri_base_demux_constructed (GObject *object)
{
  NyxUriBaseDemux *self = NYX_URI_BASE_DEMUX_CAST (object);
  GstPad *sink_pad;
  ...
  gst_pad_set_event_function (sink_pad,
      GST_DEBUG_FUNCPTR (nyx_uri_base_demux_sink_event));
  gst_pad_set_chain_function (sink_pad,
      GST_DEBUG_FUNCPTR (nyx_uri_base_demux_sink_chain));
...
static void
nyx_uri_base_demux_finalize (GObject *object)
{
  NyxUriBaseDemux *self = NYX_URI_BASE_DEMUX_CAST (object);
  NyxUriBaseDemuxPrivate *priv = nyx_uri_base_demux_get_instance_private (self);
...
static void
nyx_uri_base_demux_class_init (NyxUriBaseDemuxClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstElementClass *gstelement_class = (GstElementClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxuribasedemux", 0,
      "Nyx URI Base Demux");

  gobject_class->constructed = nyx_uri_base_demux_constructed;
  gobject_class->finalize = nyx_uri_base_demux_finalize;

  gstelement_class->change_state = nyx_uri_base_demux_change_state;
