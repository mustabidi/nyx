...
#include "gstnyxgluploader.h"

#define GST_CAT_DEFAULT gst_nyx_gl_uploader_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

#define parent_class gst_nyx_gl_uploader_parent_class
GST_NYX_IMPORTER_DEFINE (GstNyxGLUploader, gst_nyx_gl_uploader, GST_TYPE_NYX_IMPORTER);

static void
_update_elements_caps_locked (GstNyxGLUploader *self, GstCaps *upload_sink_caps)
{
  ...
  gdk_sink_caps = gst_nyx_gl_context_handler_make_gdk_gl_caps (
...
static void
gst_nyx_gl_uploader_set_caps (GstNyxImporter *importer, GstCaps *caps)
{
  GstNyxGLUploader *self = GST_NYX_GL_UPLOADER_CAST (importer);

  GST_NYX_GL_UPLOADER_LOCK (self);
  _update_elements_caps_locked (self, caps);
  GST_NYX_GL_UPLOADER_UNLOCK (self);
}

static void
_uploader_reconfigure_locked (GstNyxGLUploader *self)
{
...
static GstBuffer *
_upload_perform_locked (GstNyxGLUploader *self, GstBuffer *buffer)
{
...
gst_nyx_gl_uploader_create_pool (GstNyxImporter *importer, GstStructure **config)
{
...
static void
gst_nyx_gl_uploader_add_allocation_metas (GstNyxImporter *importer, GstQuery *query)
{
  GstNyxGLUploader *self = GST_NYX_GL_UPLOADER_CAST (importer);
  GstGLUpload *upload;
...
static GdkTexture *
gst_nyx_gl_uploader_generate_texture (GstNyxImporter *importer,
    GstBuffer *buffer, GstVideoInfo *v_info)
{
  ...
  texture = gst_nyx_gl_context_handler_make_gl_texture (self->gl_handler, color_buf, &self->v_info);
...
static void
gst_nyx_gl_uploader_init (GstNyxGLUploader *self)
{
...
static void
gst_nyx_gl_uploader_finalize (GObject *object)
{
  GstNyxGLUploader *self = GST_NYX_GL_UPLOADER_CAST (object);
...
static void
gst_nyx_gl_uploader_class_init (GstNyxGLUploaderClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstNyxImporterClass *importer_class = (GstNyxImporterClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxgluploader", 0,
      "Nyx GL Uploader");

  gobject_class->finalize = gst_nyx_gl_uploader_finalize;

  importer_class->set_caps = gst_nyx_gl_uploader_set_caps;
  importer_class->create_pool = gst_nyx_gl_uploader_create_pool;
  importer_class->add_allocation_metas = gst_nyx_gl_uploader_add_allocation_metas;
  importer_class->generate_texture = gst_nyx_gl_uploader_generate_texture;
}

GstNyxImporter *
make_importer (GPtrArray *context_handlers)
{
  GstNyxGLUploader *self;
  ...
  self = g_object_new (GST_TYPE_NYX_GL_UPLOADER, NULL);
...
static GstCaps *
_make_actual_caps (GstNyxGLContextHandler *gl_handler)
{
  GstGLUpload *upload;
...
make_caps (gboolean is_template, GstRank *rank, GPtrArray *context_handlers)
{
  ...
  if (is_template) {
    ...
    if ((gl_handler = GST_NYX_GL_CONTEXT_HANDLER_CAST (
        gst_nyx_context_handler_obtain_with_type (context_handlers,
            GST_TYPE_NYX_GL_CONTEXT_HANDLER)))) {
