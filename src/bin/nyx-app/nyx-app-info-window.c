...
#include "nyx-app-list-item-utils.h"

#define GST_CAT_DEFAULT nyx_app_info_window_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxAppInfoWindow
{
  AdwWindow parent;
...
#define parent_class nyx_app_info_window_parent_class
G_DEFINE_TYPE (NyxAppInfoWindow, nyx_app_info_window, ADW_TYPE_WINDOW);
...
static gchar *
media_duration_closure (NyxAppInfoWindow *self, gdouble duration)
{
  return g_strdup_printf ("%" NYX_TIME_MS_FORMAT, NYX_TIME_MS_ARGS (duration));
}

static gchar *
playback_element_name_closure (NyxAppInfoWindow *self, GstElement *element)
{
...
static gchar *
playback_decoder_closure (NyxAppInfoWindow *self, GstElement *decoder)
{
  GstElementFactory *factory;
  gchar *el_name, *text;
...
static gchar *
playback_sink_closure (NyxAppInfoWindow *self, GstElement *sink)
{
  GstElementFactory *factory;
  GParamSpec *pspec;
...
create_no_selection_closure (NyxAppInfoWindow *self, NyxStreamList *stream_list)
{
  return GTK_SELECTION_MODEL (gtk_no_selection_new (gst_object_ref (stream_list)));
}

static gboolean
has_streams_closure (NyxAppInfoWindow *self, guint n_streams)
{
  return (n_streams > 0);
}
...
GtkWidget *
nyx_app_info_window_new (GtkApplication *gtk_app, NyxPlayer *player)
{
  NyxAppInfoWindow *window;

  window = g_object_new (NYX_APP_TYPE_INFO_WINDOW,
      "application", gtk_app,
...
static void
nyx_app_info_window_init (NyxAppInfoWindow *self)
{
...
static void
nyx_app_info_window_dispose (GObject *object)
{
  gtk_widget_dispose_template (GTK_WIDGET (object), NYX_APP_TYPE_INFO_WINDOW);

  G_OBJECT_CLASS (parent_class)->dispose (object);
}

static void
nyx_app_info_window_finalize (GObject *object)
{
  NyxAppInfoWindow *self = NYX_APP_INFO_WINDOW_CAST (object);
...
static void
nyx_app_info_window_get_property (GObject *object, guint prop_id,
...
static void
nyx_app_info_window_set_property (GObject *object, guint prop_id,
...
static void
nyx_app_info_window_class_init (NyxAppInfoWindowClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxappinfowindow", 0,
      "Nyx App Info Window");

  gobject_class->get_property = nyx_app_info_window_get_property;
  gobject_class->set_property = nyx_app_info_window_set_property;
  gobject_class->dispose = nyx_app_info_window_dispose;
  gobject_class->finalize = nyx_app_info_window_finalize;

  gtk_widget_class_set_template_from_resource (widget_class,
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-info-window.ui");
  ...
  gtk_widget_class_bind_template_child (widget_class, NyxAppInfoWindow, vstreams_list);
  gtk_widget_class_bind_template_child (widget_class, NyxAppInfoWindow, astreams_list);
  gtk_widget_class_bind_template_child (widget_class, NyxAppInfoWindow, sstreams_list);
  gtk_widget_class_bind_template_child (widget_class, NyxAppInfoWindow, pipeline_button);
