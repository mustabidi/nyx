 ...
 *
 * By default #NyxGtkTitleLabel will automatically show title
 * of [property@Nyx.Queue:current-item] when placed within
 * [class@NyxGtk.Video] widget hierarchy.
 *
 * Setting [property@NyxGtk.TitleLabel:media-item] property will
 * make it show title of that particular [class@Nyx.MediaItem]
 * instead. Providing an item to read title from also allows using
 * this [class@Gtk.Widget] outside of [class@NyxGtk.Video].
 */
...
#define GST_CAT_DEFAULT nyx_gtk_title_label_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxGtkTitleLabel
{
  GtkWidget parent;
  ...
  NyxMediaItem *custom_item;
  gboolean fallback_to_uri;

  NyxPlayer *player;
};

#define parent_class nyx_gtk_title_label_parent_class
G_DEFINE_TYPE (NyxGtkTitleLabel, nyx_gtk_title_label, GTK_TYPE_WIDGET)
...
_label_changed_cb (GtkLabel *label G_GNUC_UNUSED,
    GParamSpec *pspec G_GNUC_UNUSED, NyxGtkTitleLabel *self)
{
  g_object_notify_by_pspec (G_OBJECT (self), param_specs[PROP_CURRENT_TITLE]);
}

static void
_refresh_title (NyxGtkTitleLabel *self)
{
  NyxMediaItem *item;
  gchar *title;
  ...
  title = nyx_media_item_get_title (item);

  if (title) {
    gtk_label_set_label (self->label, title);
    g_free (title);
  } else if (self->fallback_to_uri) {
    gtk_label_set_label (self->label, nyx_media_item_get_uri (item));
  } else {
...
static void
_media_item_title_changed_cb (NyxMediaItem *item G_GNUC_UNUSED,
    GParamSpec *pspec G_GNUC_UNUSED, NyxGtkTitleLabel *self)
{
  _refresh_title (self);
}
...
static void
_set_current_item (NyxGtkTitleLabel *self, NyxMediaItem *current_item)
{
...
static void
_bind_current_item (NyxGtkTitleLabel *self)
{
...
static void
_unbind_current_item (NyxGtkTitleLabel *self)
{
  NyxQueue *queue = nyx_player_get_queue (self->player);
...
/**
 * nyx_gtk_title_label_new:
 *
 * Creates a new #NyxGtkTitleLabel instance.
 *
 * Returns: a new title label #GtkWidget.
 */
GtkWidget *
nyx_gtk_title_label_new (void)
{
  return g_object_new (NYX_GTK_TYPE_TITLE_LABEL, NULL);
}

/**
 * nyx_gtk_title_label_set_media_item:
 * @label: a #NyxGtkTitleLabel
 * @item: (nullable): a #NyxMediaItem
 *
 ...
 */
void
nyx_gtk_title_label_set_media_item (NyxGtkTitleLabel *self, NyxMediaItem *item)
{
  g_return_if_fail (NYX_GTK_IS_TITLE_LABEL (self));
  g_return_if_fail (item == NULL || NYX_IS_MEDIA_ITEM (item));
  ...
  if (self->custom_item) {
    g_signal_connect (self->custom_item, "notify::title",
        G_CALLBACK (_media_item_title_changed_cb), self);
  } else if ((self->player = nyx_gtk_get_player_from_ancestor (GTK_WIDGET (self)))) {
...
/**
 * nyx_gtk_title_label_get_media_item:
 * @label: a #NyxGtkTitleLabel
 *
 ...
 */
NyxMediaItem *
nyx_gtk_title_label_get_media_item (NyxGtkTitleLabel *self)
{
  g_return_val_if_fail (NYX_GTK_IS_TITLE_LABEL (self), NULL);

  return self->custom_item;
}

/**
 * nyx_gtk_title_label_get_current_title:
 * @label: a #NyxGtkTitleLabel
 *
 ...
 */
const gchar *
nyx_gtk_title_label_get_current_title (NyxGtkTitleLabel *self)
{
  g_return_val_if_fail (NYX_GTK_IS_TITLE_LABEL (self), NULL);
...
/**
 * nyx_gtk_title_label_set_fallback_to_uri:
 * @label: a #NyxGtkTitleLabel
 ...
 */
void
nyx_gtk_title_label_set_fallback_to_uri (NyxGtkTitleLabel *self, gboolean enabled)
{
  g_return_if_fail (NYX_GTK_IS_TITLE_LABEL (self));
...
/**
 * nyx_gtk_title_label_get_fallback_to_uri:
 * @label: a #NyxGtkTitleLabel
 *
 ...
 */
gboolean
nyx_gtk_title_label_get_fallback_to_uri (NyxGtkTitleLabel *self)
{
  g_return_val_if_fail (NYX_GTK_IS_TITLE_LABEL (self), FALSE);

  return self->fallback_to_uri;
}

static void
nyx_gtk_title_label_init (NyxGtkTitleLabel *self)
{
...
static void
nyx_gtk_title_label_constructed (GObject *object)
{
  NyxGtkTitleLabel *self = NYX_GTK_TITLE_LABEL_CAST (object);
...
static void
nyx_gtk_title_label_compute_expand (GtkWidget *widget,
    gboolean *hexpand_p, gboolean *vexpand_p)
{
  NyxGtkTitleLabel *self = NYX_GTK_TITLE_LABEL_CAST (widget);
...
static void
nyx_gtk_title_label_root (GtkWidget *widget)
{
  NyxGtkTitleLabel *self = NYX_GTK_TITLE_LABEL_CAST (widget);
  ...
  if (!self->custom_item
      && (self->player = nyx_gtk_get_player_from_ancestor (widget))) {
...
static void
nyx_gtk_title_label_unroot (GtkWidget *widget)
{
  NyxGtkTitleLabel *self = NYX_GTK_TITLE_LABEL_CAST (widget);
...
static void
nyx_gtk_title_label_dispose (GObject *object)
{
  NyxGtkTitleLabel *self = NYX_GTK_TITLE_LABEL_CAST (object);
...
static void
nyx_gtk_title_label_get_property (GObject *object, guint prop_id,
    GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_MEDIA_ITEM:
      g_value_set_object (value, nyx_gtk_title_label_get_media_item (self));
      break;
    case PROP_CURRENT_TITLE:
      g_value_set_string (value, nyx_gtk_title_label_get_current_title (self));
      break;
    case PROP_FALLBACK_TO_URI:
      g_value_set_boolean (value, nyx_gtk_title_label_get_fallback_to_uri (self));
      break;
    default:
...
static void
nyx_gtk_title_label_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_MEDIA_ITEM:
      nyx_gtk_title_label_set_media_item (self, g_value_get_object (value));
      break;
    case PROP_FALLBACK_TO_URI:
      nyx_gtk_title_label_set_fallback_to_uri (self, g_value_get_boolean (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static void
nyx_gtk_title_label_class_init (NyxGtkTitleLabelClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxgtktitlelabel", 0,
      "Nyx GTK Title Label");
  nyx_gtk_init_translations ();

  gobject_class->constructed = nyx_gtk_title_label_constructed;
  gobject_class->get_property = nyx_gtk_title_label_get_property;
  gobject_class->set_property = nyx_gtk_title_label_set_property;
  gobject_class->dispose = nyx_gtk_title_label_dispose;

  widget_class->compute_expand = nyx_gtk_title_label_compute_expand;

  /* Using root/unroot so label "current-title" is immediately
   * updated and can be accessed before label was made visible */
  widget_class->root = nyx_gtk_title_label_root;
  widget_class->unroot = nyx_gtk_title_label_unroot;

  /**
   * NyxGtkTitleLabel:media-item:
   *
  ...
  /**
   * NyxGtkTitleLabel:current-title:
   *
  ...
  /**
   * NyxGtkTitleLabel:fallback-to-uri:
   *
  ...
  gtk_widget_class_set_css_name (widget_class, "nyx-gtk-title-label");
}
