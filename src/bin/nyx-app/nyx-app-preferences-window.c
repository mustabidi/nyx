...
#define GST_CAT_DEFAULT nyx_app_preferences_window_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxAppPreferencesWindow
{
...
#define parent_class nyx_app_preferences_window_parent_class
G_DEFINE_TYPE (NyxAppPreferencesWindow, nyx_app_preferences_window, ADW_TYPE_PREFERENCES_WINDOW);

typedef struct
{
  NyxAppPreferencesWindow *prefs;
  GHashTable *parsed_overrides;
  gboolean updated;
} NyxAppPreferencesIterRanksData;

typedef struct
{
  GSettings *settings;
  const gchar *key;
} NyxAppPreferencesResetData;
...
static void
_flag_map_data_free (NyxAppPreferencesFlagMapData *data)
{
...
static void
_reset_button_closure (NyxAppPreferencesResetData *data, GClosure *closure)
{
...
static void
_reset_button_clicked_cb (GtkButton *button, NyxAppPreferencesResetData *data)
{
...
file_selection_row_activated_cb (AdwActionRow *action_row, GParamSpec *pspec)
{
  ...
  if (pspec->flags & NYX_ENHANCER_PARAM_FILEPATH)
    nyx_app_file_dialog_select_prefs_file (gtk_app, action_row);
  else
    nyx_app_file_dialog_select_prefs_dir (gtk_app, action_row);
}
...
static gboolean
_get_flag_mapping (GValue *value, GVariant *variant, NyxAppPreferencesFlagMapData *data)
{
...
static GVariant *
_set_flag_mapping (GValue *value, GVariantType *exp_type, NyxAppPreferencesFlagMapData *data)
{
...
static gboolean
_add_enhancer_config_row (NyxAppPreferencesWindow *self, GParamSpec *pspec,
    GSettings *enhancer_settings)
{
  GtkWidget *row = NULL, *reset_button;
  NyxAppPreferencesResetData *reset_data;
  ...
  switch (pspec->value_type) {
    ...
    case G_TYPE_STRING:{
      if (pspec->flags & (NYX_ENHANCER_PARAM_FILEPATH | NYX_ENHANCER_PARAM_DIRPATH)) {
        GtkWidget *image;
  ...
  reset_data = g_new (NyxAppPreferencesResetData, 1);
...
selected_enhancer_changed_cb (AdwComboRow *combo_row,
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppPreferencesWindow *self)
{
  ...
  if (selected != GTK_INVALID_LIST_POSITION) {
    NyxEnhancerProxyList *proxies = nyx_get_global_enhancer_proxies ();
    NyxEnhancerProxy *proxy = nyx_enhancer_proxy_list_peek_proxy (proxies, selected);
    GParamSpec **pspecs;
    guint n_pspecs;
    gboolean has_props = FALSE;

    if ((pspecs = nyx_enhancer_proxy_get_target_properties (proxy, &n_pspecs))) {
      GSettings *enhancer_settings = NULL;
      guint i;

      for (i = 0; i < n_pspecs; ++i) {
        if (pspecs[i]->flags & NYX_ENHANCER_PARAM_GLOBAL) {
          if (!enhancer_settings)
            enhancer_settings = nyx_enhancer_proxy_get_settings (proxy);
          if (enhancer_settings)
...
static void
enhancers_config_activated_cb (AdwActionRow *action_row, NyxAppPreferencesWindow *self)
{
  /* If no model set yet */
  if (!adw_combo_row_get_model (self->enhancers_combo_row)) {
    ...
    if (nyx_enhancer_proxy_list_get_n_proxies (proxies) > 0)
...
static gboolean
_find_rank_overide_for_name (NyxAppPreferencesWindow *self,
...
static void
remove_rank_override_button_clicked_cb (GtkButton *button, NyxAppPreferencesWindow *self)
{
  GtkWidget *spin_row;
...
static void
_add_rank_override (NyxAppPreferencesWindow *self,
...
_iter_ranks_func (const gchar *feature_name, GstRank rank,
    gboolean from_env, NyxAppPreferencesIterRanksData *data)
{
  NyxAppPreferencesWindow *self = data->prefs;
  guint index = 0;
...
static void
_update_rank_overrides (NyxAppPreferencesWindow *self)
{
  NyxAppPreferencesIterRanksData *data;
  gint i;

  data = g_new (NyxAppPreferencesIterRanksData, 1);
  data->prefs = self;
  ...
  nyx_app_utils_iterate_plugin_feature_ranks (self->settings,
      (NyxAppUtilsIterRanks) _iter_ranks_func, data);
...
static void
add_override_button_clicked_cb (GtkButton *button, NyxAppPreferencesWindow *self)
{
...
_make_plugin_features_string_list (NyxAppPreferencesWindow *self, const gchar *plugin_name)
{
  GList *features, *feat;
...
static gboolean
list_has_selection_closure (NyxAppPreferencesWindow *self, guint selected)
{
  return (selected != GTK_INVALID_LIST_POSITION);
}

static GtkStringList *
ranking_features_model_closure (NyxAppPreferencesWindow *self, GtkStringObject *string_obj)
{
...
static gboolean
add_override_button_sensitive_closure (NyxAppPreferencesWindow *self,
...
plugin_feature_ranks_settings_changed_cb (GSettings *settings,
    gchar *key G_GNUC_UNUSED, NyxAppPreferencesWindow *self)
{
...
static void
_ensure_plugins_and_features_lists (NyxAppPreferencesWindow *self)
{
  GList *feat;
  GStrvBuilder *builder;
  gchar **plugin_names;
...
static void
plugin_ranking_activated_cb (AdwActionRow *action_row, NyxAppPreferencesWindow *self)
{
...
static void
plugin_ranking_unrealize_cb (GtkWidget *widget, NyxAppPreferencesWindow *self)
{
  GString *string;
  gchar *ranks_str;
  guint i;
...
GtkWidget *
nyx_app_preferences_window_new (GtkApplication *gtk_app)
{
  NyxAppPreferencesWindow *window;

  window = g_object_new (NYX_APP_TYPE_PREFERENCES_WINDOW,
      "application", gtk_app,
...
static void
nyx_app_preferences_window_init (NyxAppPreferencesWindow *self)
{
#if NYX_HAVE_SERVER
  ...
  if ((proxy = nyx_enhancer_proxy_list_get_proxy_by_module (
      nyx_get_global_enhancer_proxies (), "nyx-control-hub"))) {
...
static void
nyx_app_preferences_window_dispose (GObject *object)
{
  NyxAppPreferencesWindow *self = NYX_APP_PREFERENCES_WINDOW_CAST (object);

  g_clear_pointer (&self->rank_rows, g_ptr_array_unref);
  gtk_widget_dispose_template (GTK_WIDGET (self), NYX_APP_TYPE_PREFERENCES_WINDOW);

  G_OBJECT_CLASS (parent_class)->dispose (object);
}

static void
nyx_app_preferences_window_finalize (GObject *object)
{
  NyxAppPreferencesWindow *self = NYX_APP_PREFERENCES_WINDOW_CAST (object);
...
static void
nyx_app_preferences_get_property (GObject *object, guint prop_id,
...
static void
nyx_app_preferences_window_class_init (NyxAppPreferencesWindowClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxapppreferenceswindow", 0,
      "Nyx App Preferences Window");

  gobject_class->get_property = nyx_app_preferences_get_property;
  gobject_class->dispose = nyx_app_preferences_window_dispose;
  gobject_class->finalize = nyx_app_preferences_window_finalize;

  gtk_widget_class_set_template_from_resource (widget_class,
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-preferences-window.ui");
  ...
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, seek_method_combo_row);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, seek_unit_combo_row);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, seek_value_spin_row);

  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, features_group);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, server_switch_row);

  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, audio_offset_spin_row);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, subtitle_offset_spin_row);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, font_dialog_button);

  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, enhancers_stack);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, browse_enhancers_page);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, no_enhancers_page);

  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, enhancers_subpage);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, enhancers_combo_row);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, enhancer_config_group);

  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, plugins_subpage);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, plugins_combo_row);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, features_combo_row);
  gtk_widget_class_bind_template_child (widget_class, NyxAppPreferencesWindow, overrides_group);
