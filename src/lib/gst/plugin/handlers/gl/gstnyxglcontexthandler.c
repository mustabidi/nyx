...
#include "gstnyxglcontexthandler.h"
#include "gst/plugin/gstgdkformats.h"
#include "gst/plugin/gstgtkutils.h"

#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_WAYLAND
#include <gdk/wayland/gdkwayland.h>
#include <gst/gl/wayland/gstgldisplay_wayland.h>
#endif

#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_X11
#include <gdk/x11/gdkx.h>
#endif
#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_X11_GLX
#include <gst/gl/x11/gstgldisplay_x11.h>
#endif
#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_X11_EGL || GST_NYX_GL_CONTEXT_HANDLER_HAVE_WIN32_EGL
#include <gst/gl/egl/gstgldisplay_egl.h>
#endif

#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_WIN32
#include <gdk/win32/gdkwin32.h>
#endif

#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_MACOS
#include <gdk/macos/gdkmacos.h>
#endif

#define GST_CAT_DEFAULT gst_nyx_gl_context_handler_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

#define parent_class gst_nyx_gl_context_handler_parent_class
G_DEFINE_TYPE (GstNyxGLContextHandler, gst_nyx_gl_context_handler, GST_TYPE_OBJECT);
...
static gboolean
_gl_context_handler_context_realize (GstNyxGLContextHandler *self, GdkGLContext *gdk_context)
{
...
#endif
#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_X11_EGL
  if (GDK_IS_X11_DISPLAY (gdk_display) && gdk_x11_display_get_egl_display (gdk_display))
    preferred_api = GDK_GL_API_GLES;
#endif
#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_WIN32_EGL
...
#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_WAYLAND || GST_NYX_GL_CONTEXT_HANDLER_HAVE_X11_EGL
...
static gboolean
_retrieve_gl_context_on_main (GstNyxGLContextHandler *self)
{
  GdkDisplay *gdk_display;
...
#endif
#if GST_NYX_GL_CONTEXT_HANDLER_HAVE_WIN32_WGL
    if (!self->gst_display) {
      self->gst_display =
...
static gboolean
_retrieve_gst_context (GstNyxGLContextHandler *self)
{
...
static gboolean
gst_nyx_gl_context_handler_handle_context_query (GstNyxContextHandler *handler,
    GstBaseSink *bsink, GstQuery *query)
{
  GstNyxGLContextHandler *self = GST_NYX_GL_CONTEXT_HANDLER_CAST (handler);
  gboolean res;
...
static void
gst_nyx_gl_context_handler_init (GstNyxGLContextHandler *self)
{
}

static void
gst_nyx_gl_context_handler_constructed (GObject *object)
{
  GstNyxGLContextHandler *self = GST_NYX_GL_CONTEXT_HANDLER_CAST (object);
...
static void
gst_nyx_gl_context_handler_finalize (GObject *object)
{
  GstNyxGLContextHandler *self = GST_NYX_GL_CONTEXT_HANDLER_CAST (object);
...
static void
gst_nyx_gl_context_handler_class_init (GstNyxGLContextHandlerClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstNyxContextHandlerClass *handler_class = (GstNyxContextHandlerClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxglcontexthandler", 0,
      "Nyx GL Context Handler");

  gobject_class->constructed = gst_nyx_gl_context_handler_constructed;
  gobject_class->finalize = gst_nyx_gl_context_handler_finalize;

  handler_class->handle_context_query = gst_nyx_gl_context_handler_handle_context_query;
}

void
gst_nyx_gl_context_handler_add_handler (GPtrArray *context_handlers)
{
  guint i;
  ...
  for (i = 0; i < context_handlers->len; i++) {
    ...
    if ((found = GST_IS_NYX_GL_CONTEXT_HANDLER (handler)))
      break;
  }

  if (!found) {
    GstNyxGLContextHandler *handler = g_object_new (
        GST_TYPE_NYX_GL_CONTEXT_HANDLER, NULL);
...
GstCaps *
gst_nyx_gl_context_handler_make_gdk_gl_caps (const gchar *features, gboolean only_2d)
{
  GstCaps *caps, *tmp;
...
GdkTexture *
gst_nyx_gl_context_handler_make_gl_texture (GstNyxGLContextHandler *self,
