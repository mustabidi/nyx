 ...
 * NyxHarvest:
 *
 * An object storing data from enhancers that implement [iface@Nyx.Extractable] interface.
 *
 * Since: 0.8
 */
...
#define GST_CAT_DEFAULT nyx_harvest_debug
...
#define parent_class nyx_harvest_parent_class
G_DEFINE_TYPE (NyxHarvest, nyx_harvest, GST_TYPE_OBJECT);

static inline void
_ensure_tags (NyxHarvest *self)
{
  if (!self->tags) {
...
static inline void
_ensure_headers (NyxHarvest *self)
{
  if (!self->headers)
...
NyxHarvest *
nyx_harvest_new (void)
{
  NyxHarvest *harvest;

  harvest = g_object_new (NYX_TYPE_HARVEST, NULL);
...
void
nyx_harvest_set_enhancer_in_caps (NyxHarvest *self, NyxEnhancerProxy *proxy)
{
  gst_caps_set_simple (self->caps, "enhancer", G_TYPE_STRING,
      nyx_enhancer_proxy_get_module_name (proxy), NULL);
}

gboolean
nyx_harvest_unpack (NyxHarvest *self,
...
static void
_harvest_fill_toc_from_cache (NyxHarvest *self, const gchar **data)
{
  guint i, n_entries;

  n_entries = nyx_cache_read_uint (data);
  for (i = 0; i < n_entries; ++i) {
    guint j, n_subentries;

    n_subentries = nyx_cache_read_uint (data);
    for (j = 0; j < n_subentries; ++j) {
      GstTocEntryType type;
      const gchar *title;
      gdouble start, end;

      type = (GstTocEntryType) nyx_cache_read_int (data);
      title = nyx_cache_read_string (data);
      start = nyx_cache_read_double (data);
      end = nyx_cache_read_double (data);

      nyx_harvest_toc_add (self, type, title, start, end);
    }
  }
}

static void
_harvest_store_toc_to_cache (NyxHarvest *self, GByteArray *bytes)
{
  GList *list = NULL, *el;
  guint n_entries = 0;
  ...
  nyx_cache_store_uint (bytes, n_entries);
  ...
  for (el = list; el; el = g_list_next (el)) {
    ...
    nyx_cache_store_uint (bytes, n_subentries);
    ...
    for (sub_el = subentries; sub_el; sub_el = g_list_next (sub_el)) {
      ...
      nyx_cache_store_int (bytes, (gint) gst_toc_entry_get_entry_type (subentry));

      if ((tags = gst_toc_entry_get_tags (subentry)))
        gst_tag_list_peek_string_index (tags, GST_TAG_TITLE, 0, &title);

      nyx_cache_store_string (bytes, title);
      ...
      nyx_cache_store_double (bytes, start_dbl);
      nyx_cache_store_double (bytes, end_dbl);
    }
  }
}

static inline gchar *
_build_cache_filename (NyxEnhancerProxy *proxy, GUri *uri)
{
...
gboolean
nyx_harvest_fill_from_cache (NyxHarvest *self, NyxEnhancerProxy *proxy,
    const GstStructure *config, GUri *uri)
{
  ...
  if (G_LIKELY ((epoch_cached = nyx_cache_read_int64 (&data)) > 0)) {
  ...
  GST_DEBUG_OBJECT (self, "Cached harvest expiration in %" NYX_TIME_FORMAT,
      NYX_TIME_ARGS (exp_seconds));

  /* Read last used config to generate cache data */
  if ((read_str = nyx_cache_read_string (&data)))
  ...
  /* Read caps */
  read_str = nyx_cache_read_string (&data);
  ...
  /* Read buffer data */
  buf_data = nyx_cache_read_data (&data, &buf_size);
...
void
nyx_harvest_export_to_cache (NyxHarvest *self, NyxEnhancerProxy *proxy,
    const GstStructure *config, GUri *uri)
{
  GByteArray *bytes;
  ...
  nyx_cache_store_string (bytes, nyx_enhancer_proxy_get_version (proxy));

  /* Store expiration date */
  nyx_cache_store_int64 (bytes, self->exp_epoch);

  /* Store config used to generate harvest */
  if (config)
    temp_str = gst_structure_to_string (config);
  nyx_cache_store_string (bytes, temp_str); // NULL when no config
  g_clear_pointer (&temp_str, g_free);

  /* Store caps */
  temp_str = gst_caps_to_string (self->caps);
  if (G_LIKELY (temp_str != NULL)) {
    nyx_cache_store_string (bytes, temp_str);
    g_clear_pointer (&temp_str, g_free);
  } else {
  ...
  if (G_LIKELY (data_ok)) {
    ...
    if (G_LIKELY (gst_memory_map (mem, &map_info, GST_MAP_READ))) {
      nyx_cache_store_data (bytes, map_info.data, map_info.size);
 ...
 *
 *   * `application/nyx-playlist` - custom playlist format
 *
 ...
 */
gboolean
nyx_harvest_fill (NyxHarvest *self, const gchar *media_type, gpointer data, gsize size)
{
  g_return_val_if_fail (NYX_IS_HARVEST (self), FALSE);
  ...
  self->caps = gst_caps_new_simple (media_type,
      "source", G_TYPE_STRING, "nyx-harvest", NULL);

  return TRUE;
}

/**
 * nyx_harvest_fill_with_text:
 * @harvest: a #NyxHarvest
 * @media_type: media mime type
 ...
 *
 * For more info, see [method@Nyx.Harvest.fill] documentation.
 *
 * Returns: %TRUE when filled successfully, %FALSE if taken data was empty.
 *
 * Since: 0.8
 */
gboolean
nyx_harvest_fill_with_text (NyxHarvest *self, const gchar *media_type, gchar *text)
{
  g_return_val_if_fail (text != NULL, FALSE);

  return nyx_harvest_fill (self, media_type, text, strlen (text));
}

/**
 * nyx_harvest_fill_with_bytes:
 * @harvest: a #NyxHarvest
 * @media_type: media mime type
 ...
 */
gboolean
nyx_harvest_fill_with_bytes (NyxHarvest *self, const gchar *media_type, GBytes *bytes)
{
  gpointer data;
  gsize size = 0;
  ...
  return nyx_harvest_fill (self, media_type, data, size);
}

/**
 * nyx_harvest_tags_add:
 * @harvest: a #NyxHarvest
 ...
 */
void
nyx_harvest_tags_add (NyxHarvest *self, const gchar *tag, ...)
{
  va_list args;

  g_return_if_fail (NYX_IS_HARVEST (self));
...
/**
 * nyx_harvest_tags_add_value: (rename-to nyx_harvest_tags_add)
 * @harvest: a #NyxHarvest
 * @tag: a name of tag to set
 ...
 */
void
nyx_harvest_tags_add_value (NyxHarvest *self, const gchar *tag, const GValue *value)
{
  g_return_if_fail (NYX_IS_HARVEST (self));
  g_return_if_fail (tag != NULL);
 ...
 */
void
nyx_harvest_toc_add (NyxHarvest *self, GstTocEntryType type,
 ...
 */
void
nyx_harvest_headers_set (NyxHarvest *self, const gchar *key, ...)
{
  va_list args;
...
/**
 * nyx_harvest_headers_set_value: (rename-to nyx_harvest_headers_set)
 * @harvest: a #NyxHarvest
 * @key: a header name
 ...
 */
void
nyx_harvest_headers_set_value (NyxHarvest *self, const gchar *key, const GValue *value)
{
  g_return_if_fail (NYX_IS_HARVEST (self));
  g_return_if_fail (key != NULL);
...
/**
 * nyx_harvest_set_expiration_date_utc:
 * @harvest: a #NyxHarvest
 ...
 * the need to run [vfunc@Nyx.Extractable.extract] again.
 *
 * Since: 0.10
 */
void
nyx_harvest_set_expiration_date_utc (NyxHarvest *self, GDateTime *date_utc)
{
  g_return_if_fail (NYX_IS_HARVEST (self));
...
/**
 * nyx_harvest_set_expiration_seconds:
 * @harvest: a #NyxHarvest
 ...
 *
 * Alternative function to [method@Nyx.Harvest.set_expiration_date_utc],
 ...
 */
void
nyx_harvest_set_expiration_seconds (NyxHarvest *self, gdouble seconds)
{
  GDateTime *date, *date_epoch;

  g_return_if_fail (NYX_IS_HARVEST (self));

  GST_LOG_OBJECT (self, "Set expiration in %" NYX_TIME_FORMAT,
      NYX_TIME_ARGS (seconds));
...
static void
nyx_harvest_init (NyxHarvest *self)
{
}

static void
nyx_harvest_finalize (GObject *object)
{
  NyxHarvest *self = NYX_HARVEST_CAST (object);
...
static void
nyx_harvest_class_init (NyxHarvestClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxharvest", 0,
      "Nyx Harvest");

  gobject_class->finalize = nyx_harvest_finalize;
}
