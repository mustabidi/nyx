...
#define NYX_PLAYLIST_MEDIA_TYPE "application/nyx-playlist"
#define NYX_CLAPS_MEDIA_TYPE "text/nyx-claps"
#define URI_LIST_MEDIA_TYPE "text/uri-list"
#define DATA_CHUNK_SIZE 4096

#define NTH_REDIRECT_STRUCTURE_NAME "NyxQueryNthRedirect"
#define NTH_REDIRECT_FIELD "nth-redirect"
#define MAX_REDIRECTS 10

#define GST_CAT_DEFAULT nyx_playlist_demux_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxPlaylistDemux
{
...
static GstStaticPadTemplate sink_template = GST_STATIC_PAD_TEMPLATE ("sink",
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS (NYX_PLAYLIST_MEDIA_TYPE ";" NYX_CLAPS_MEDIA_TYPE ";" URI_LIST_MEDIA_TYPE));

static GstStaticCaps nyx_playlist_caps = GST_STATIC_CAPS (NYX_PLAYLIST_MEDIA_TYPE);
static GstStaticCaps nyx_claps_caps = GST_STATIC_CAPS (NYX_CLAPS_MEDIA_TYPE);

static void
nyx_playlist_type_find (GstTypeFind *tf, NyxEnhancerProxy *proxy)
{
  ...
  if ((prefix = nyx_enhancer_proxy_get_extra_data (proxy, "X-Data-Prefix"))) {
  ...
  contains = nyx_enhancer_proxy_get_extra_data (proxy, "X-Data-Contains");
  excludes = nyx_enhancer_proxy_get_extra_data (proxy, "X-Data-Excludes");
  regex = nyx_enhancer_proxy_get_extra_data (proxy, "X-Data-Regex");
  ...
  module_name = nyx_enhancer_proxy_get_module_name (proxy);
  GST_INFO ("Suggesting likely type: " NYX_PLAYLIST_MEDIA_TYPE
      ", enhancer: %s", module_name);

  gst_type_find_suggest_simple (tf, GST_TYPE_FIND_LIKELY,
      NYX_PLAYLIST_MEDIA_TYPE, "enhancer", G_TYPE_STRING, module_name, NULL);
}
...
static void
nyx_claps_type_find (GstTypeFind *tf, gpointer user_data G_GNUC_UNUSED)
{
  const guint8 *data;
  ...
  if (_is_claps_possible (data, data_size)) {
    ...
    if (data) {
      ...
      GST_INFO ("Suggesting %s type: " NYX_CLAPS_MEDIA_TYPE,
          (probability >= GST_TYPE_FIND_LIKELY) ? "likely" : "possible");
      gst_type_find_suggest_empty_simple (tf, probability, NYX_CLAPS_MEDIA_TYPE);
    }
  }
}
...
{
  NyxEnhancerProxyList *global_proxies = nyx_get_global_enhancer_proxies ();
  GstCaps *reg_caps;
  guint i, n_proxies = nyx_enhancer_proxy_list_get_n_proxies (global_proxies);
  gboolean res;

  reg_caps = gst_static_caps_get (&nyx_claps_caps);
  res = gst_type_find_register (plugin, "nyx-claps",
      GST_RANK_MARGINAL + 1, (GstTypeFindFunction) nyx_claps_type_find,
      "claps", reg_caps, NULL, NULL);
  gst_clear_caps (&reg_caps);

  for (i = 0; i < n_proxies; ++i) {
    NyxEnhancerProxy *proxy = nyx_enhancer_proxy_list_peek_proxy (global_proxies, i);

    /* No "X-Data-Excludes" check here, because it can not be
     * used alone to determine whether data is a playlist */
    if (nyx_enhancer_proxy_target_has_interface (proxy, NYX_TYPE_PLAYLISTABLE)
        && (nyx_enhancer_proxy_get_extra_data (proxy, "X-Data-Prefix")
        || nyx_enhancer_proxy_get_extra_data (proxy, "X-Data-Contains")
        || nyx_enhancer_proxy_get_extra_data (proxy, "X-Data-Regex"))) {
      if (!reg_caps)
        reg_caps = gst_static_caps_get (&nyx_playlist_caps);

      res |= gst_type_find_register (plugin, nyx_enhancer_proxy_get_module_name (proxy),
          GST_RANK_MARGINAL + 1, (GstTypeFindFunction) nyx_playlist_type_find,
...
#define parent_class nyx_playlist_demux_parent_class
G_DEFINE_TYPE (NyxPlaylistDemux, nyx_playlist_demux, NYX_TYPE_URI_BASE_DEMUX);
GST_TYPE_FIND_REGISTER_DEFINE_CUSTOM (nyxplaylistdemux, type_find_register);
GST_ELEMENT_REGISTER_DEFINE (nyxplaylistdemux, "nyxplaylistdemux",
    512, NYX_TYPE_PLAYLIST_DEMUX);

static GListStore *
_parse_uri_list (NyxPlaylistDemux *self, GUri *uri, GstBuffer *buffer,
    GCancellable *cancellable, GError **error)
{
  ...
  playlist = g_list_store_new (NYX_TYPE_MEDIA_ITEM);
  ptr = (gchar *) info.data;
  end = ptr + info.size;
  ...
  while (ptr < end) {
    ...
    if (gst_uri_is_valid (line)) {
      ...
      if (res_uri) {
        ...
        item = nyx_media_item_new (res_uri);
        g_free (res_uri);
      }
    }
...
static void
nyx_playlist_demux_handle_caps (NyxUriBaseDemux *uri_bd, GstCaps *caps)
{
  NyxPlaylistDemux *self = NYX_PLAYLIST_DEMUX_CAST (uri_bd);
...
static GList *
_filter_playlistables (NyxPlaylistDemux *self, GstCaps *caps, NyxEnhancerProxyList *proxies)
{
  GList *sublist = NULL;
  GstStructure *structure;
  ...
  if (caps && (structure = gst_caps_get_structure (self->caps, 0))) {
    ...
    if (module_name && (proxy = nyx_enhancer_proxy_list_get_proxy_by_module (proxies, module_name)))
...
_handle_playlist (NyxPlaylistDemux *self, GListStore *playlist, GCancellable *cancellable)
{
  NyxMediaItem *item;
  GstStructure *structure;
  const gchar *uri;
  ...
  structure = gst_structure_new ("NyxPlaylistParsed",
  ...
  success = nyx_uri_base_demux_set_uri (NYX_URI_BASE_DEMUX_CAST (self), uri, NULL);
  gst_object_unref (item);
...
static gboolean
nyx_playlist_demux_handle_custom_query (NyxUriBaseDemux *uri_bd, GstQuery *query)
{
  NyxPlaylistDemux *self = NYX_PLAYLIST_DEMUX_CAST (uri_bd);
  const GstStructure *structure = gst_query_get_structure (query);
...
static gboolean
nyx_playlist_demux_process_buffer (NyxUriBaseDemux *uri_bd,
    GstBuffer *buffer, GCancellable *cancellable)
{
  ...
  if (_caps_have_media_type (self->caps, NYX_PLAYLIST_MEDIA_TYPE)) {
    ...
    if (G_LIKELY (self->enhancer_proxies != NULL)) {
      ...
      /* Compat for old NyxDiscoverer feature that does not set this property */
      GST_WARNING_OBJECT (self, "Falling back to using global enhancer proxy list!");
      proxies = gst_object_ref (nyx_get_global_enhancer_proxies ());
    }

    GST_OBJECT_UNLOCK (self);

    if (!self->director)
      self->director = nyx_enhancer_director_new ();

    filtered_proxies = _filter_playlistables (self, self->caps, proxies);
    gst_object_unref (proxies);

    playlist = nyx_enhancer_director_parse (self->director,
    ...
    g_clear_list (&filtered_proxies, gst_object_unref);
      ...
      || _caps_have_media_type (self->caps, NYX_CLAPS_MEDIA_TYPE)) {
...
static void
nyx_playlist_demux_set_enhancer_proxies (NyxPlaylistDemux *self,
    NyxEnhancerProxyList *enhancer_proxies)
{
  GST_OBJECT_LOCK (self);
...
static void
nyx_playlist_demux_init (NyxPlaylistDemux *self)
{
}

static void
nyx_playlist_demux_dispose (GObject *object)
{
  NyxPlaylistDemux *self = NYX_PLAYLIST_DEMUX_CAST (object);
...
static void
nyx_playlist_demux_finalize (GObject *object)
{
  NyxPlaylistDemux *self = NYX_PLAYLIST_DEMUX_CAST (object);
...
static void
nyx_playlist_demux_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_ENHANCER_PROXIES:
      nyx_playlist_demux_set_enhancer_proxies (self, g_value_get_object (value));
      break;
    default:
...
static void
nyx_playlist_demux_class_init (NyxPlaylistDemuxClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstElementClass *gstelement_class = (GstElementClass *) klass;
  NyxUriBaseDemuxClass *nyxuribd_class = (NyxUriBaseDemuxClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxplaylistdemux", 0,
      "Nyx Playlist Demux");

  gobject_class->set_property = nyx_playlist_demux_set_property;
  gobject_class->dispose = nyx_playlist_demux_dispose;
  gobject_class->finalize = nyx_playlist_demux_finalize;

  nyxuribd_class->handle_caps = nyx_playlist_demux_handle_caps;
  nyxuribd_class->handle_custom_query = nyx_playlist_demux_handle_custom_query;
  nyxuribd_class->process_buffer = nyx_playlist_demux_process_buffer;
  ...
  gst_element_class_set_static_metadata (gstelement_class, "Nyx Playlist Demux",
