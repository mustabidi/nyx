...
#define GST_CAT_DEFAULT gst_nyx_importer_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

#define parent_class gst_nyx_importer_parent_class
G_DEFINE_TYPE (GstNyxImporter, gst_nyx_importer, GST_TYPE_OBJECT);
...
static GstNyxGdkOverlay *
gst_nyx_gdk_overlay_new (GdkTexture *texture, GstVideoOverlayRectangle *rectangle,
    gint x, gint y, guint width, guint height, guint index)
{
  GstNyxGdkOverlay *overlay = g_slice_new (GstNyxGdkOverlay);
...
static GstNyxGdkOverlay *
gst_nyx_gdk_overlay_ref (GstNyxGdkOverlay *overlay)
{
...
static void
gst_nyx_gdk_overlay_unref (GstNyxGdkOverlay *overlay)
{
  if (g_atomic_ref_count_dec (&overlay->ref_count)) {
    ...
    g_slice_free (GstNyxGdkOverlay, overlay);
  }
}

static GstBufferPool *
_default_create_pool (GstNyxImporter *self, GstStructure **config)
{
...
static void
_default_add_allocation_metas (GstNyxImporter *importer, GstQuery *query)
{
...
static GdkTexture *
_default_generate_texture (GstNyxImporter *self,
...
static void
gst_nyx_importer_init (GstNyxImporter *self)
{
  ...
  self->pending_overlays = g_ptr_array_new_with_free_func (
      (GDestroyNotify) gst_nyx_gdk_overlay_unref);
...
static void
gst_nyx_importer_finalize (GObject *object)
{
  GstNyxImporter *self = GST_NYX_IMPORTER_CAST (object);
...
static void
gst_nyx_importer_class_init (GstNyxImporterClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstNyxImporterClass *importer_class = (GstNyxImporterClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyximporter", 0,
      "Nyx Importer");

  gobject_class->finalize = gst_nyx_importer_finalize;
...
_get_cached_overlay (GPtrArray *overlays, GstVideoOverlayRectangle *rectangle)
{
  ...
  for (i = 0; i < overlays->len; i++) {
    GstNyxGdkOverlay *overlay = g_ptr_array_index (overlays, i);
...
{
  GstNyxGdkOverlay *overlay_a, *overlay_b;

  overlay_a = *((GstNyxGdkOverlay **) a);
  overlay_b = *((GstNyxGdkOverlay **) b);
 ...
 */
static void
gst_nyx_importer_prepare_overlays_locked (GstNyxImporter *self)
{
  ...
  for (i = 0; i < self->pending_overlays->len; i++) {
    GstNyxGdkOverlay *overlay = g_ptr_array_index (self->pending_overlays, i);
    overlay->index = -1;
  }
  ...
  for (i = 0; i < num_overlays; i++) {
    ...
    if ((texture = gst_video_frame_into_gdk_texture (&comp_frame))) {
      overlay = gst_nyx_gdk_overlay_new (texture, rectangle, comp_x, comp_y,
  ...
  for (i = self->pending_overlays->len; i > 0; i--) {
    GstNyxGdkOverlay *overlay = g_ptr_array_index (self->pending_overlays, i - 1);
...
void
gst_nyx_importer_set_caps (GstNyxImporter *self, GstCaps *caps)
{
  GstNyxImporterClass *importer_class = GST_NYX_IMPORTER_GET_CLASS (self);
...
void
gst_nyx_importer_set_buffer (GstNyxImporter *self, GstBuffer *buffer)
{
  GST_OBJECT_LOCK (self);
  ...
  gst_nyx_importer_prepare_overlays_locked (self);

  GST_OBJECT_UNLOCK (self);
}

GstBufferPool *
gst_nyx_importer_create_pool (GstNyxImporter *self, GstStructure **config)
{
  GstNyxImporterClass *importer_class = GST_NYX_IMPORTER_GET_CLASS (self);
...
void
gst_nyx_importer_add_allocation_metas (GstNyxImporter *self, GstQuery *query)
{
  GstNyxImporterClass *importer_class = GST_NYX_IMPORTER_GET_CLASS (self);
...
void
gst_nyx_importer_snapshot (GstNyxImporter *self, GdkSnapshot *snapshot,
    gdouble width, gdouble height)
{
  ...
  for (i = 0; i < self->pending_overlays->len; i++) {
    ...
    g_ptr_array_insert (self->overlays, i, gst_nyx_gdk_overlay_ref (overlay));
  }
    ...
    if (G_LIKELY (self->texture)) {
      ...
      if (self->overlays->len > 0) {
        ...
        for (i = 0; i < self->overlays->len; i++) {
          GstNyxGdkOverlay *overlay = g_ptr_array_index (self->overlays, i);
