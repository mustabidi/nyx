...
_open_uri_cb (AdwAlertDialog *dialog, GAsyncResult *result, GtkApplication *gtk_app)
{
  ...
  if (strcmp (response, "add") == 0) {
    ...
    if (nyx_app_utils_files_from_string (text, &files, &n_files)) {
...
void
nyx_app_uri_dialog_open_uri (GtkApplication *gtk_app)
{
  ...
  builder = gtk_builder_new_from_resource (
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-uri-dialog.ui");
