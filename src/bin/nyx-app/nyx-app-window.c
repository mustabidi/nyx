...
#define NYX_APP_SEEK_UNIT_SECOND 0
#define NYX_APP_SEEK_UNIT_MINUTE 1
#define NYX_APP_SEEK_UNIT_PERCENTAGE 2
...
#define GST_CAT_DEFAULT nyx_app_window_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxAppWindow
{
  GtkApplicationWindow parent;

  GtkWidget *video;
  NyxGtkBillboard *billboard;
  NyxGtkSimpleControls *simple_controls;
...
#define parent_class nyx_app_window_parent_class
G_DEFINE_TYPE (NyxAppWindow, nyx_app_window, GTK_TYPE_APPLICATION_WINDOW)

typedef struct
{
  gint dest_width, dest_height;
  gint64 last_tick;
} NyxAppWindowResizeData;

static guint16 instance_count = 0;

static inline GQuark
nyx_app_window_extra_options_get_quark (void)
{
  return g_quark_from_static_string ("nyx-app-window-extra-options-quark");
}

static void
nyx_app_window_extra_options_free (NyxAppWindowExtraOptions *extra_opts)
{
...
static void
_media_item_title_changed_cb (NyxMediaItem *item,
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppWindow *self)
{
  gchar *title;

  if ((title = nyx_media_item_get_title (item))) {
    gtk_window_set_title (GTK_WINDOW (self), title);
    g_free (title);
  } else {
    gtk_window_set_title (GTK_WINDOW (self), NYX_APP_NAME);
  }
}
...
_queue_current_item_changed_cb (NyxQueue *queue,
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppWindow *self)
{
  NyxMediaItem *current_item = nyx_queue_get_current_item (queue);
...
static void
_player_adaptive_bandwidth_changed_cb (NyxPlayer *player,
    GParamSpec *pspec G_GNUC_UNUSED, gpointer *user_data G_GNUC_UNUSED)
{
  /* Do not take whole bandwidth */
  nyx_player_set_adaptive_start_bitrate (player,
      nyx_player_get_adaptive_bandwidth (player) * 0.8);
}
    ...
    GVariant *variant, gpointer user_data G_GNUC_UNUSED)
{
  ...
  seek_method = (NyxPlayerSeekMethod) g_variant_get_int32 (variant);
...
static GtkWidget *
_pick_pointer_widget (NyxAppWindow *self)
{
...
static void
_player_volume_changed_cb (NyxPlayer *player,
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppWindow *self)
{
  gdouble volume = PERCENTAGE_ROUND (nyx_player_get_volume (player));
  ...
  if (volume != self->last_volume) {
    nyx_gtk_billboard_announce_volume (self->billboard);
    self->last_volume = volume;
  }
}

static void
_player_speed_changed_cb (NyxPlayer *player G_GNUC_UNUSED,
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppWindow *self)
{
  nyx_gtk_billboard_announce_speed (self->billboard);
}

static void
video_toggle_fullscreen_cb (NyxGtkVideo *video, NyxAppWindow *self)
{
...
static void
video_map_cb (GtkWidget *widget, NyxAppWindow *self)
{
  NyxPlayer *player;
  gdouble speed;
  ...
  player = nyx_gtk_av_get_player (NYX_GTK_AV_CAST (self->video));
...
static void
video_unmap_cb (GtkWidget *widget, NyxAppWindow *self)
{
  NyxPlayer *player;
...
static void
_open_subtitles_cb (NyxGtkExtraMenuButton *button G_GNUC_UNUSED,
    NyxMediaItem *item, NyxAppWindow *self)
{
  GtkApplication *gtk_app = gtk_window_get_application (GTK_WINDOW (self));
  nyx_app_file_dialog_open_subtitles (gtk_app, item);
}

static void
click_pressed_cb (GtkGestureClick *click, gint n_press,
    gdouble x, gdouble y, NyxAppWindow *self)
{
  GdkCursor *cursor;
...
_resize_tick (GtkWidget *widget, GdkFrameClock *frame_clock,
    NyxAppWindowResizeData *resize_data)
{
  gint64 now = gdk_frame_clock_get_frame_time (frame_clock);

  if (now - resize_data->last_tick >= MIN_STEP_DELAY) {
    NyxAppWindow *self = NYX_APP_WINDOW_CAST (widget);
...
static void
_resize_window (NyxAppWindow *self)
{
  NyxPlayer *player;
  NyxStreamList *vstreams;
  NyxVideoStream *vstream;
  ...
  player = nyx_app_window_get_player (self);
  vstreams = nyx_player_get_video_streams (player);
  vstream = NYX_VIDEO_STREAM_CAST (
      nyx_stream_list_get_current_stream (vstreams));

  if (vstream) {
    gint video_width = nyx_video_stream_get_width (vstream);
    gint video_height = nyx_video_stream_get_height (vstream);
    ...
    if (G_LIKELY (video_width > 0 && video_height > 0)) {
      ...
      if (dest_width != win_width || dest_height != win_height) {
        ...
        resize_data = g_new0 (NyxAppWindowResizeData, 1);
...
static void
_handle_middle_click (NyxAppWindow *self, GtkGestureClick *click)
{
  _resize_window (self);
...
static void
_handle_right_click (NyxAppWindow *self, GtkGestureClick *click)
{
  GdkSurface *surface;
...
drag_begin_cb (GtkGestureDrag *drag,
    gdouble start_x, gdouble start_y, NyxAppWindow *self)
{
...
drag_update_cb (GtkGestureDrag *drag,
    gdouble offset_x, gdouble offset_y, NyxAppWindow *self)
{
...
static inline void
_alter_volume (NyxAppWindow *self, gdouble dy)
{
  NyxPlayer *player = nyx_gtk_av_get_player (NYX_GTK_AV_CAST (self->video));
  gdouble volume = nyx_player_get_volume (player);
  ...
  nyx_player_set_volume (player, PERCENTAGE_ROUND (volume));
}

static inline void
_alter_speed (NyxAppWindow *self, gdouble dx)
{
  NyxPlayer *player = nyx_gtk_av_get_player (NYX_GTK_AV_CAST (self->video));
  gdouble speed = nyx_player_get_speed (player);
  ...
  nyx_player_set_speed (player, PERCENTAGE_ROUND (speed));
}

static gboolean
_begin_seek_operation (NyxAppWindow *self)
{
  NyxPlayer *player;
  NyxQueue *queue;
  NyxMediaItem *current_item;
  ...
  self->current_duration = (current_item != NULL)
      ? nyx_media_item_get_duration (current_item)
      : 0;
  ...
  if ((self->was_playing = (
      nyx_player_get_state (player) == NYX_PLAYER_STATE_PLAYING)))
    nyx_player_pause (player);

  self->pending_position = nyx_player_get_position (player);
  self->seeking = TRUE;
...
static void
_end_seek_operation (NyxAppWindow *self)
{
  if (self->seeking && self->current_duration > 0) {
    NyxPlayer *player = nyx_gtk_av_get_player (
        NYX_GTK_AV_CAST (self->video));

    nyx_player_seek_custom (player, self->pending_position,
...
static void
_announce_current_seek_position (NyxAppWindow *self, gboolean forward)
{
  gchar *position_str = g_strdup_printf (
      "%" NYX_TIME_FORMAT " / %" NYX_TIME_FORMAT,
      NYX_TIME_ARGS (self->pending_position),
      NYX_TIME_ARGS (self->current_duration));

  nyx_gtk_billboard_post_message (self->billboard,
...
static inline void
_alter_position (NyxAppWindow *self, gdouble dx)
{
  gboolean forward;
...
static void
scroll_begin_cb (GtkEventControllerScroll *scroll, NyxAppWindow *self)
{
...
scroll_cb (GtkEventControllerScroll *scroll,
    gdouble dx, gdouble dy, NyxAppWindow *self)
{
  GtkWidget *pickup;
  GdkDevice *device;
  gboolean handled;
  ...
  while (pickup && !NYX_GTK_IS_VIDEO (pickup)) {
...
static void
scroll_end_cb (GtkEventControllerScroll *scroll, NyxAppWindow *self)
{
...
static void
_handle_seek_key_press (NyxAppWindow *self, gboolean forward)
{
  gint unit;
  gdouble offset;
  ...
  switch (unit) {
    case NYX_APP_SEEK_UNIT_SECOND:
      break;
    case NYX_APP_SEEK_UNIT_MINUTE:
      offset *= 60;
      break;
    case NYX_APP_SEEK_UNIT_PERCENTAGE:
...
static void
_handle_chapter_key_press (NyxAppWindow *self, gboolean forward)
{
  NyxPlayer *player = nyx_gtk_av_get_player (
      NYX_GTK_AV_CAST (self->video));
  ...
  timeline = nyx_media_item_get_timeline (current_item);
  i = nyx_timeline_get_n_markers (timeline);
  ...
  while (i--) {
    NyxMarker *marker = nyx_timeline_get_marker (timeline, i);
    NyxMarkerType marker_type = nyx_marker_get_marker_type (marker);
    gdouble start;
    gboolean found = FALSE;

    /* Ignore custom markers */
    if (marker_type >= NYX_MARKER_TYPE_CUSTOM_1) {
      gst_object_unref (marker);
      continue;
    }

    start = nyx_marker_get_start (marker);
  ...
  if (dest_marker) {
    ...
    title = nyx_marker_get_title (dest_marker);
    start = nyx_marker_get_start (dest_marker);
    duration = nyx_media_item_get_duration (current_item);

    /* XXX: When RTL with mixed numbers and text, we have to
     * switch positions of start <-> duration ourselves */
    text = g_strdup_printf (
        "%s\n%" NYX_TIME_FORMAT " / %" NYX_TIME_FORMAT, title,
        NYX_TIME_ARGS ((!is_rtl) ? start : duration),
        NYX_TIME_ARGS ((!is_rtl) ? duration : start));

    nyx_gtk_billboard_post_message (self->billboard,
        "user-bookmarks-symbolic", text);
    nyx_player_seek (player, start);
...
static void
_handle_item_key_press (NyxAppWindow *self, gboolean forward)
{
  NyxPlayer *player = nyx_gtk_av_get_player (
      NYX_GTK_AV_CAST (self->video));
  NyxQueue *queue = nyx_player_get_queue (player);
  ...
  prev_index = nyx_queue_get_current_index (queue);
  gtk_widget_activate_action (self->video,
      (forward) ? "av.next-item" : "av.previous-item", NULL);
  index = nyx_queue_get_current_index (queue);
...
static void
_handle_speed_key_press (NyxAppWindow *self, gboolean forward)
{
  forward ^= (gtk_widget_get_default_direction () == GTK_TEXT_DIR_RTL);
...
static void
_handle_advance_frame_key_press (NyxAppWindow *self)
{
  NyxPlayer *player = nyx_gtk_av_get_player (
      NYX_GTK_AV_CAST (self->video));
  ...
  if (state >= NYX_PLAYER_STATE_PAUSED)
    nyx_player_advance_frame (player);
}

static inline void
_handle_progression_key_press (NyxAppWindow *self)
{
  NyxPlayer *player = nyx_gtk_av_get_player (
      NYX_GTK_AV_CAST (self->video));
  NyxQueue *queue = nyx_player_get_queue (player);
  ...
  mode = ((nyx_queue_get_progression_mode (queue) + 1) % N_PROGRESSION_MODES);

  nyx_app_utils_parse_progression (mode, &icon, &label);
  nyx_queue_set_progression_mode (queue, mode);

  nyx_gtk_billboard_post_message (self->billboard, icon, label);
}

static gboolean
key_pressed_cb (GtkEventControllerKey *controller, guint keyval,
    guint keycode, GdkModifierType state, NyxAppWindow *self)
{
  switch (keyval) {
    case GDK_KEY_Up:
...
static void
_seek_delay_cb (NyxAppWindow *self)
{
...
static void
video_seek_request_cb (NyxGtkVideo *video, gboolean forward, NyxAppWindow *self)
{
    ...
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppWindow *self)
{
  ...
  if (!value) {
    nyx_gtk_billboard_unpin_pinned_message (self->billboard);
    return;
  }

  if (!nyx_app_utils_value_for_item_is_valid (value)) {
  ...
  if (gtk_stack_get_visible_child (GTK_STACK (stack)) == self->video) {
    nyx_gtk_billboard_pin_message (self->billboard,
    ...
    gdouble x, gdouble y, NyxAppWindow *self)
{
  ...
  if (nyx_app_utils_files_from_value (value, &files, &n_files)) {
    NyxPlayer *player = nyx_app_window_get_player (self);
    NyxQueue *queue = nyx_player_get_queue (player);
    gboolean start_playback = (nyx_queue_get_n_items (queue) == 0);

    nyx_app_window_ensure_no_initial_state (self);
...
{
  NyxAppWindow *self = NYX_APP_WINDOW_CAST (user_data);

  video_toggle_fullscreen_cb (NYX_GTK_VIDEO_CAST (self->video), self);
}
...
unfullscreen (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  if (gtk_window_is_fullscreen (window)) {
    NyxAppWindow *self = NYX_APP_WINDOW_CAST (window);
    video_toggle_fullscreen_cb (NYX_GTK_VIDEO_CAST (self->video), self);
  }
}
...
{
  _resize_window (NYX_APP_WINDOW_CAST (user_data));
}
...
show_help_overlay (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  builder = gtk_builder_new_from_resource (
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-help-overlay.ui");
...
GtkWidget *
nyx_app_window_new (GtkApplication *application)
{
  return g_object_new (NYX_APP_TYPE_WINDOW,
      "application", application,
      NULL);
}

GtkWidget *
nyx_app_window_get_video (NyxAppWindow *self)
{
  return self->video;
}

NyxPlayer *
nyx_app_window_get_player (NyxAppWindow *self)
{
  return nyx_gtk_av_get_player (NYX_GTK_AV_CAST (self->video));
}

NyxAppWindowExtraOptions *
nyx_app_window_get_extra_options (NyxAppWindow *self)
{
  return g_object_get_qdata ((GObject *) self,
      nyx_app_window_extra_options_get_quark ());
}

void
nyx_app_window_ensure_no_initial_state (NyxAppWindow *self)
{
...
static gboolean
nyx_app_window_close_request (GtkWindow *window)
{
  NyxAppWindow *self = NYX_APP_WINDOW_CAST (window);
/* FIXME: Have GSettings again to store these values
  GSettings *settings = g_settings_new (NYX_APP_ID);
...
static void
nyx_app_window_realize (GtkWidget *widget)
{
  NyxAppWindow *self = NYX_APP_WINDOW_CAST (widget);
...
static void
nyx_app_window_unrealize (GtkWidget *widget)
{
  NyxAppWindow *self = NYX_APP_WINDOW_CAST (widget);
...
static void
nyx_app_window_init (NyxAppWindow *self)
{
  NyxAppWindowExtraOptions *extra_opts;
  ...
  extra_opts = g_new0 (NyxAppWindowExtraOptions, 1);
  GST_TRACE ("Created window extra options: %p", extra_opts);

  g_object_set_qdata_full ((GObject *) self,
      nyx_app_window_extra_options_get_quark (),
      extra_opts, (GDestroyNotify) nyx_app_window_extra_options_free);
...
static void
nyx_app_window_constructed (GObject *object)
{
  NyxAppWindow *self = NYX_APP_WINDOW_CAST (object);
  NyxPlayer *player = nyx_app_window_get_player (self);
  NyxQueue *queue = nyx_player_get_queue (player);
  NyxEnhancerProxyList *proxies = nyx_player_get_enhancer_proxies (player);
  NyxEnhancerProxy *proxy;
  NyxGtkExtraMenuButton *button;
...
#if (NYX_HAVE_MPRIS || NYX_HAVE_SERVER || NYX_HAVE_DISCOVERER)
  NyxFeature *feature = NULL;
#endif

  gchar mpris_name[45];
  g_snprintf (mpris_name, sizeof (mpris_name),
      "org.mpris.MediaPlayer2.Nyx.instance%" G_GUINT16_FORMAT, instance_count++);

  self->settings = g_settings_new (NYX_APP_ID);
  self->last_volume = PERCENTAGE_ROUND (g_settings_get_double (self->settings, "volume"));

  if ((proxy = nyx_enhancer_proxy_list_get_proxy_by_module (proxies, "nyx-mpris"))) {
    nyx_enhancer_proxy_set_locally (proxy,
        "app-id", NYX_APP_ID,
        "own-name", mpris_name,
        "identity", NYX_APP_NAME,
        "desktop-entry", NYX_APP_ID,
        "queue-controllable", TRUE, NULL);
    gst_object_unref (proxy);
  } else {
#if NYX_HAVE_MPRIS
    feature = NYX_FEATURE (nyx_mpris_new (
        mpris_name, NYX_APP_NAME, NYX_APP_ID));
    nyx_mpris_set_queue_controllable (NYX_MPRIS (feature), TRUE);
    nyx_player_add_feature (player, feature);
    gst_object_unref (feature);
#endif
  }

  if ((proxy = nyx_enhancer_proxy_list_get_proxy_by_module (proxies, "nyx-control-hub"))) {
    nyx_enhancer_proxy_set_locally (proxy,
        "queue-controllable", TRUE, NULL);
    gst_object_unref (proxy);
  } else {
#if NYX_HAVE_SERVER
    feature = NYX_FEATURE (nyx_server_new ());
    nyx_server_set_queue_controllable (NYX_SERVER (feature), TRUE);
  ...
  if ((proxy = nyx_enhancer_proxy_list_get_proxy_by_module (proxies, "nyx-media-scanner"))) {
    gst_object_unref (proxy);
  } else {
    feature = NYX_FEATURE (nyx_discoverer_new ());
    nyx_player_add_feature (player, feature);
    gst_object_unref (feature);
  }
#endif
  ...
  nyx_player_set_autoplay (player, TRUE);
  ...
  button = nyx_gtk_simple_controls_get_extra_menu_button (
  ...
  nyx_gtk_extra_menu_button_set_can_open_subtitles (button, TRUE);
  ...
  gtk_css_provider_load_from_resource (self->provider,
      NYX_APP_RESOURCE_PREFIX "/css/styles.css");
...
static void
nyx_app_window_dispose (GObject *object)
{
  NyxAppWindow *self = NYX_APP_WINDOW_CAST (object);
  ...
  gtk_widget_dispose_template (GTK_WIDGET (object), NYX_APP_TYPE_WINDOW);
...
static void
nyx_app_window_finalize (GObject *object)
{
  NyxAppWindow *self = NYX_APP_WINDOW_CAST (object);
...
static void
nyx_app_window_class_init (NyxAppWindowClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;
  GtkWindowClass *window_class = (GtkWindowClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxappwindow", 0,
      "Nyx App Window");

  gobject_class->constructed = nyx_app_window_constructed;
  gobject_class->dispose = nyx_app_window_dispose;
  gobject_class->finalize = nyx_app_window_finalize;

  widget_class->realize = nyx_app_window_realize;
  widget_class->unrealize = nyx_app_window_unrealize;

  window_class->close_request = nyx_app_window_close_request;

  gtk_widget_class_set_template_from_resource (widget_class,
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-window.ui");

  gtk_widget_class_bind_template_child (widget_class, NyxAppWindow, video);
  gtk_widget_class_bind_template_child (widget_class, NyxAppWindow, billboard);
  gtk_widget_class_bind_template_child (widget_class, NyxAppWindow, simple_controls);
  gtk_widget_class_bind_template_child (widget_class, NyxAppWindow, drop_target);
