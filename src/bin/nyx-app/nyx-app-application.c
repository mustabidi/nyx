...
#include "nyx-app-uri-dialog.h"
#include "nyx-app-info-window.h"
...
#define GST_CAT_DEFAULT nyx_app_application_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxAppApplication
{
...
#define parent_class nyx_app_application_parent_class
G_DEFINE_TYPE (NyxAppApplication, nyx_app_application, GTK_TYPE_APPLICATION);

struct NyxPluginFeatureData
{
  const gchar *name;
  GstRank rank;
};

struct NyxPluginData
{
  const gchar *name;
  guint skip_version[3];
  struct NyxPluginFeatureData features[10];
};

typedef struct
{
  NyxAppApplication *app;
  guint id;
} NyxAppWindowData;
...
static gboolean
nyx_app_options_get (const gchar *key, const gchar *format, GVariantDict *options,
...
static gboolean
nyx_app_options_get_extra (const gchar *key, GVariantDict *options,
 ...
 */
static void
nyx_app_apply_options_to_window (NyxAppWindow *dest_window, GVariantDict *options,
    NyxAppWindow *src_window, GSettings *settings)
{
  NyxPlayer *dest_player;
  NyxAppWindowExtraOptions *src_extra_opts = NULL, *dest_extra_opts = NULL;
  ...
  dest_player = nyx_app_window_get_player (dest_window);
  dest_extra_opts = nyx_app_window_get_extra_options (dest_window);

  if (src_window) {
    src_player_obj = (GObject *) nyx_app_window_get_player (src_window);
    src_queue_obj = (GObject *) nyx_player_get_queue (NYX_PLAYER_CAST (src_player_obj));
    src_extra_opts = nyx_app_window_get_extra_options (src_window);
  }

  /* Apply player values, clamp them to be within allowed range */
  if (nyx_app_options_get ("volume", "d", options, src_player_obj, settings, &option_dbl))
    nyx_player_set_volume (dest_player, PERCENTAGE_ROUND (CLAMP (option_dbl, 0, 2.0)));
  if (nyx_app_options_get ("mute", "b", NULL, src_player_obj, settings, &option_bool))
    nyx_player_set_mute (dest_player, option_bool);
  if (nyx_app_options_get ("speed", "d", options, src_player_obj, settings, &option_dbl))
    nyx_player_set_speed (dest_player, PERCENTAGE_ROUND (CLAMP (option_dbl, 0.05, 2.0)));
  if (nyx_app_options_get ("adaptive-start-bitrate", "i", options, src_player_obj, settings, &option_int))
    nyx_player_set_adaptive_start_bitrate (dest_player, option_int);
  if (nyx_app_options_get ("progression-mode", "i", options, src_queue_obj, settings, &option_int))
    nyx_queue_set_progression_mode (nyx_player_get_queue (dest_player), CLAMP (option_int, 0, 4));
  if (nyx_app_options_get ("subtitles-enabled", "b", NULL, src_player_obj, settings, &option_bool))
    nyx_player_set_subtitles_enabled (dest_player, option_bool);

  if (nyx_app_options_get_extra ("video-filter", options,
      (src_extra_opts) ? src_extra_opts->video_filter : NULL, NULL, &option_str)) {
    nyx_player_set_video_filter (dest_player, nyx_app_utils_make_element (option_str));
    g_free (dest_extra_opts->video_filter);
    dest_extra_opts->video_filter = option_str;
  }
  if (nyx_app_options_get_extra ("audio-filter", options,
      (src_extra_opts) ? src_extra_opts->audio_filter : NULL, NULL, &option_str)) {
    nyx_player_set_audio_filter (dest_player, nyx_app_utils_make_element (option_str));
    g_free (dest_extra_opts->audio_filter);
    dest_extra_opts->audio_filter = option_str;
  }
  if (nyx_app_options_get_extra ("video-sink", options,
      (src_extra_opts) ? src_extra_opts->video_sink : NULL, NULL, &option_str)) {
    nyx_player_set_video_sink (dest_player, nyx_app_utils_make_element (option_str));
    g_free (dest_extra_opts->video_sink);
    dest_extra_opts->video_sink = option_str;
  }
  if (nyx_app_options_get_extra ("audio-sink", options,
      (src_extra_opts) ? src_extra_opts->audio_sink : NULL, NULL, &option_str)) {
    nyx_player_set_audio_sink (dest_player, nyx_app_utils_make_element (option_str));
...
static inline void
_store_settings_from_window (NyxAppApplication *self, NyxAppWindow *app_window)
{
  NyxPlayer *player = nyx_app_window_get_player (app_window);
  ...
  g_settings_set_double (self->settings, "volume", nyx_player_get_volume (player));
  g_settings_set_boolean (self->settings, "mute", nyx_player_get_mute (player));
  g_settings_set_double (self->settings, "speed", nyx_player_get_speed (player));
  g_settings_set_int (self->settings, "adaptive-start-bitrate",
      CLAMP (nyx_player_get_adaptive_bandwidth (player) * 0.8, 0, G_MAXINT));
  g_settings_set_boolean (self->settings, "subtitles-enabled", nyx_player_get_subtitles_enabled (player));
  g_settings_set_int (self->settings, "progression-mode", nyx_queue_get_progression_mode (queue));
...
_set_initial_plugin_feature_ranks (void)
{
  ...
  const struct NyxPluginData plugins_data[] = {
    {
      .name = "va",
      .skip_version = { 1, 24, 0 },
      .features = {
...
{
  nyx_app_utils_iterate_plugin_feature_ranks (settings,
      (NyxAppUtilsIterRanks) _iter_ranks_func, NULL);
}
...
_assemble_initial_state (GtkWindow *window)
{
  ...
  GtkBuilder *builder = gtk_builder_new_from_resource (
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-initial-state.ui");
...
add_files (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  nyx_app_file_dialog_open_files (gtk_app);
}

static void
add_folder (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  GtkApplication *gtk_app = GTK_APPLICATION (user_data);

  nyx_app_file_dialog_open_folder (gtk_app);
}
...
add_uri (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  nyx_app_uri_dialog_open_uri (gtk_app);
}
...
clear_queue (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  while (window && !NYX_APP_IS_WINDOW (window))
    window = gtk_window_get_transient_for (window);

  player = nyx_app_window_get_player (NYX_APP_WINDOW (window));
  queue = nyx_player_get_queue (player);

  nyx_queue_clear (queue);
}
...
new_window (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  if (g_strcmp0 (child_name, "initial_state") != 0) {
    NyxAppWindow *src_window, *dest_window;

    src_window = NYX_APP_WINDOW_CAST (gtk_application_get_active_window (gtk_app));
    dest_window = NYX_APP_WINDOW_CAST (nyx_app_window_new (gtk_app));

    nyx_app_apply_options_to_window (dest_window, NULL, src_window, NULL);
...
show_preferences (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  preferences_window = nyx_app_preferences_window_new (gtk_app);
...
show_info (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  info_window = nyx_app_info_window_new (gtk_app, player);
  gtk_window_present (GTK_WINDOW (info_window));
}

static void
_launch_pipeline_cb (GtkFileLauncher *launcher,
    GAsyncResult *res, NyxAppWindowData *win_data)
{
  GError *error = NULL;
...
{
  NyxAppApplication *self = NYX_APP_APPLICATION_CAST (user_data);
  GtkApplication *gtk_app = GTK_APPLICATION (self);
  GtkWindow *window;
  NyxAppWindowData *win_data;
  ...
  win_data = g_new (NyxAppWindowData, 1);
  win_data->app = self;
  win_data->id = gtk_application_window_get_id (GTK_APPLICATION_WINDOW (window));

  nyx_app_utils_create_pipeline_svg_file_async (
      nyx_app_window_get_player (NYX_APP_WINDOW (window)),
...
show_about (GSimpleAction *action, GVariant *param, gpointer user_data)
{
  ...
  window = gtk_application_get_active_window (gtk_app);
  about_dialog = nyx_app_about_dialog_new ();
  adw_dialog_present (ADW_DIALOG (about_dialog), GTK_WIDGET (window));
}

GApplication *
nyx_app_application_new (void)
{
  return g_object_new (NYX_APP_TYPE_APPLICATION,
      "application-id", NYX_APP_ID,
      "flags", G_APPLICATION_HANDLES_OPEN | G_APPLICATION_HANDLES_COMMAND_LINE,
      NULL);
}

static void
nyx_app_application_window_removed (GtkApplication *gtk_app, GtkWindow *window)
{
  NyxAppApplication *self = NYX_APP_APPLICATION_CAST (gtk_app);

  if (NYX_APP_IS_WINDOW (window)) {
    ...
    for (win = windows; win != NULL; win = win->next) {
      ...
      if ((has_player_windows = (rem_window != window
          && NYX_APP_IS_WINDOW (rem_window))))
        break;
    }

    /* Last player window is closing, time to store settings */
    if (!has_player_windows)
      _store_settings_from_window (self, NYX_APP_WINDOW_CAST (window));
  }
...
static void
nyx_app_application_activate (GApplication *app)
{
  NyxAppApplication *self = NYX_APP_APPLICATION_CAST (app);
  ...
  if (!(window = gtk_application_get_active_window (gtk_app))) {
    window = GTK_WINDOW (nyx_app_window_new (gtk_app));
    nyx_app_apply_options_to_window (NYX_APP_WINDOW_CAST (window),
...
static gboolean
nyx_app_application_local_command_line (GApplication *app,
...
static gint
nyx_app_application_command_line (GApplication *app, GApplicationCommandLine *cmd_line)
{
  NyxAppApplication *self = NYX_APP_APPLICATION_CAST (app);
  NyxAppWindow *src_window = NULL, *dest_window = NULL;
  GtkApplication *gtk_app = GTK_APPLICATION (app);
  ...
  dest_window = NYX_APP_WINDOW_CAST (gtk_application_get_active_window (gtk_app));
  ...
  nyx_app_apply_options_to_window (dest_window, options, src_window, settings);

  if (nyx_app_utils_files_from_command_line (cmd_line, &files, &n_files)) {
...
static void
add_item_from_file (GFile *file, NyxQueue *queue)
{
...
add_item_with_subtitles (GFile *media_file,
    GFile *subs_file, NyxQueue *queue)
{
  NyxMediaItem *item = nyx_media_item_new_from_file (media_file);
  gchar *suburi = g_file_get_uri (subs_file);

  GST_DEBUG ("Adding media item with URI: %s, SUBURI: %s",
      nyx_media_item_get_uri (item), GST_STR_NULL (suburi));
...
static void
nyx_app_application_open (GApplication *app,
    GFile **files, gint n_files, const gchar *hint)
{
  ...
  n_before = nyx_queue_get_n_items (queue);

  /* Special path for opening video with subtitles at once */
  if (n_files == 2) {
    gboolean first_subs, second_subs;

    first_subs = nyx_app_utils_is_subtitles_file (files[0]);
    second_subs = nyx_app_utils_is_subtitles_file (files[1]);
  ...
  if (!handled) {
    ...
    start_playback = (n_before == 0 && !add_only);
    ...
    if (!add_only && n_before > 0)
      nyx_queue_select_index (queue, n_before);
  }
...
static void
nyx_app_application_init (NyxAppApplication *self)
{
  self->need_init_state = TRUE;
}

static void
nyx_app_application_constructed (GObject *object)
{
  NyxAppApplication *self = NYX_APP_APPLICATION_CAST (object);
  ...
  static const GActionEntry app_actions[] = {
    ...
    { "add-folder", add_folder, NULL, NULL, NULL },
  ...
  static const NyxAppShortcut app_shortcuts[] = {
    { "app.add-files", { "<Control>o", NULL, NULL }},
    { "app.add-folder", { "<Control><Shift>o", "<Control>d", NULL }},
...
static void
nyx_app_application_dispose (GObject *object)
{
  NyxAppApplication *self = NYX_APP_APPLICATION_CAST (object);
...
static void
nyx_app_application_finalize (GObject *object)
{
  NyxAppApplication *self = NYX_APP_APPLICATION_CAST (object);
...
static void
nyx_app_application_class_init (NyxAppApplicationClass *klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxappapplication", 0,
      "Nyx App Application");

  gobject_class->constructed = nyx_app_application_constructed;
  gobject_class->dispose = nyx_app_application_dispose;
  gobject_class->finalize = nyx_app_application_finalize;

  gtk_application_class->window_removed = nyx_app_application_window_removed;

  application_class->activate = nyx_app_application_activate;
  application_class->local_command_line = nyx_app_application_local_command_line;
  application_class->command_line = nyx_app_application_command_line;
  application_class->open = nyx_app_application_open;
}
