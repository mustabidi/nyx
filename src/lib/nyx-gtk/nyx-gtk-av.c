 ...
 *
 * See its descendants: [class@NyxGtk.Audio] and [class@NyxGtk.Video].
 *
 * # Actions
 *
 * #NyxGtkAv defines a set of built-in actions:
 *
 * ```yaml
...
#define GST_CAT_DEFAULT nyx_gtk_av_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

typedef struct _NyxGtkAvPrivate NyxGtkAvPrivate;

struct _NyxGtkAvPrivate
{
  NyxPlayer *player;
...
#define parent_class nyx_gtk_av_parent_class
G_DEFINE_TYPE_WITH_PRIVATE (NyxGtkAv, nyx_gtk_av, GTK_TYPE_WIDGET)
...
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (widget);
  NyxPlayer *player = nyx_gtk_av_get_player (self);
...
seek_custom_action_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (widget);
  NyxPlayer *player = nyx_gtk_av_get_player (self);
  NyxPlayerSeekMethod method = NYX_PLAYER_SEEK_METHOD_NORMAL;
  gdouble position = 0;

  g_variant_get (parameter, "(di)", &position, &method);
  nyx_player_seek_custom (player, position, method);
}
...
volume_up_action_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (widget);
  NyxPlayer *player = nyx_gtk_av_get_player (self);
  gdouble volume = (nyx_player_get_volume (player) + 0.02);
...
speed_up_action_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (widget);
  NyxPlayer *player = nyx_gtk_av_get_player (self);
  gdouble dest, speed = nyx_player_get_speed (player);
  ...
  nyx_player_set_speed (player, dest);
}
...
previous_item_action_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
  ...
  nyx_queue_select_previous_item (nyx_player_get_queue (player));
}
...
next_item_action_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
  ...
  nyx_queue_select_next_item (nyx_player_get_queue (player));
}
...
select_item_action_cb (GtkWidget *widget, const gchar *action_name, GVariant *parameter)
{
  ...
  nyx_queue_select_index (nyx_player_get_queue (player), index);
}
...
_ensure_resources (void)
{
  ...
  if (G_LIKELY (display != NULL)) {
    ...
    gtk_css_provider_load_from_resource (provider,
        NYX_GTK_RESOURCE_PREFIX "/css/styles.css");
    gtk_style_context_add_provider_for_display (display,
        (GtkStyleProvider *) provider, GTK_STYLE_PROVIDER_PRIORITY_APPLICATION - 1);
    g_object_unref (provider);

    icon_theme = gtk_icon_theme_get_for_display (display);
    gtk_icon_theme_add_resource_path (icon_theme, NYX_GTK_RESOURCE_PREFIX "/icons");
...
static inline void
_set_inhibit_session (NyxGtkAv *self, gboolean inhibit)
{
  NyxGtkAvPrivate *priv = nyx_gtk_av_get_instance_private (self);
  GtkRoot *root;
  GApplication *app;
...
_player_state_changed_cb (NyxPlayer *player,
    GParamSpec *pspec G_GNUC_UNUSED, NyxGtkAv *self)
{
  NyxGtkAvPrivate *priv = nyx_gtk_av_get_instance_private (self);

  if (priv->auto_inhibit) {
    NyxPlayerState state = nyx_player_get_state (player);
    _set_inhibit_session (self, state == NYX_PLAYER_STATE_PLAYING);
  }
}

/**
 * nyx_gtk_av_get_player:
 * @av: a #NyxGtkAv
 *
 * Get #NyxPlayer used by this #NyxGtkAv instance.
 *
 * Returns: (transfer none): a #NyxPlayer used by widget.
 *
 * Since: 0.10
 */
NyxPlayer *
nyx_gtk_av_get_player (NyxGtkAv *self)
{
  NyxGtkAvPrivate *priv;

  g_return_val_if_fail (NYX_GTK_IS_AV (self), NULL);

  priv = nyx_gtk_av_get_instance_private (self);

  return priv->player;
}

/**
 * nyx_gtk_av_set_auto_inhibit:
 * @av: a #NyxGtkAv
 ...
 */
void
nyx_gtk_av_set_auto_inhibit (NyxGtkAv *self, gboolean inhibit)
{
  NyxGtkAvPrivate *priv;

  g_return_if_fail (NYX_GTK_IS_AV (self));
 ...
 */
gboolean
nyx_gtk_av_get_auto_inhibit (NyxGtkAv *self)
{
  NyxGtkAvPrivate *priv;

  g_return_val_if_fail (NYX_GTK_IS_AV (self), FALSE);
 ...
 */
gboolean
nyx_gtk_av_get_inhibited (NyxGtkAv *self)
{
  NyxGtkAvPrivate *priv;
...
static void
nyx_gtk_av_root (GtkWidget *widget)
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (widget);
  NyxGtkAvPrivate *priv = nyx_gtk_av_get_instance_private (self);
  ...
  if (priv->auto_inhibit) {
    NyxPlayerState state = nyx_player_get_state (priv->player);
    _set_inhibit_session (self, state == NYX_PLAYER_STATE_PLAYING);
  }
}

static void
nyx_gtk_av_unroot (GtkWidget *widget)
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (widget);
...
static void
nyx_gtk_av_constructed (GObject *object)
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (object);
  NyxGtkAvPrivate *priv = nyx_gtk_av_get_instance_private (self);
  GstElement *afilter;

  priv->player = nyx_player_new ();
  ...
  if (G_LIKELY (afilter != NULL))
    nyx_player_set_audio_filter (priv->player, afilter);

  G_OBJECT_CLASS (parent_class)->constructed (object);
}

static void
nyx_gtk_av_dispose (GObject *object)
{
  NyxGtkAv *self = NYX_GTK_AV_CAST (object);
  NyxGtkAvPrivate *priv = nyx_gtk_av_get_instance_private (self);
...
static void
nyx_gtk_av_get_property (GObject *object, guint prop_id,
    GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_PLAYER:
      g_value_set_object (value, nyx_gtk_av_get_player (self));
      break;
    case PROP_AUTO_INHIBIT:
      g_value_set_boolean (value, nyx_gtk_av_get_auto_inhibit (self));
      break;
    case PROP_INHIBITED:
      g_value_set_boolean (value, nyx_gtk_av_get_inhibited (self));
      break;
    default:
...
static void
nyx_gtk_av_set_property (GObject *object, guint prop_id,
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_AUTO_INHIBIT:
      nyx_gtk_av_set_auto_inhibit (self, g_value_get_boolean (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static void
nyx_gtk_av_class_init (NyxGtkAvClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxgtkav", GST_DEBUG_FG_MAGENTA,
      "Nyx GTK AV");

  widget_class->root = nyx_gtk_av_root;
  widget_class->unroot = nyx_gtk_av_unroot;

  gobject_class->constructed = nyx_gtk_av_constructed;
  gobject_class->get_property = nyx_gtk_av_get_property;
  gobject_class->set_property = nyx_gtk_av_set_property;
  gobject_class->dispose = nyx_gtk_av_dispose;

  /**
   * NyxGtkAv:player:
   *
   * A #NyxPlayer used by widget.
   */
  param_specs[PROP_PLAYER] = g_param_spec_object ("player",
      NULL, NULL, NYX_TYPE_PLAYER,
      G_PARAM_READABLE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);

  /**
   * NyxGtkAv:auto-inhibit:
   *
  ...
  gtk_widget_class_set_css_name (widget_class, "nyx-gtk-av");
}
