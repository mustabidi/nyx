...
#define GST_CAT_DEFAULT nyx_app_window_state_buttons_debug
GST_DEBUG_CATEGORY_STATIC (GST_CAT_DEFAULT);

struct _NyxAppWindowStateButtons
{
  GtkBox parent;
...
#define parent_class nyx_app_window_state_buttons_parent_class
G_DEFINE_TYPE (NyxAppWindowStateButtons, nyx_app_window_state_buttons, GTK_TYPE_BOX)

static GParamSpec *param_specs[PROP_LAST] = { NULL, };

static void
minimize_button_clicked_cb (GtkButton *button, NyxAppWindowStateButtons *self)
{
...
static void
maximize_button_clicked_cb (GtkButton *button, NyxAppWindowStateButtons *self)
{
...
static void
close_button_clicked_cb (GtkButton *button, NyxAppWindowStateButtons *self)
{
...
static void
_refresh_buttons_visibility (NyxAppWindowStateButtons *self)
{
...
static void
nyx_app_window_state_buttons_parse_layout (NyxAppWindowStateButtons *self)
{
...
_decoration_layout_changed_cb (GtkSettings *settings,
    GParamSpec *pspec G_GNUC_UNUSED, NyxAppWindowStateButtons *self)
{
  nyx_app_window_state_buttons_parse_layout (self);
...
static void
nyx_app_window_state_buttons_init (NyxAppWindowStateButtons *self)
{
...
static void
nyx_app_window_state_buttons_constructed (GObject *object)
{
  NyxAppWindowStateButtons *self = NYX_APP_WINDOW_STATE_BUTTONS_CAST (object);
...
static void
_clear_stored_settings (NyxAppWindowStateButtons *self)
{
  if (self->settings) {
...
static void
nyx_app_window_state_buttons_realize (GtkWidget *widget)
{
  NyxAppWindowStateButtons *self = NYX_APP_WINDOW_STATE_BUTTONS_CAST (widget);
  GtkSettings *settings;
  GtkRoot *root;
  GdkSurface *surface;
...
static void
nyx_app_window_state_buttons_unrealize (GtkWidget *widget)
{
  NyxAppWindowStateButtons *self = NYX_APP_WINDOW_STATE_BUTTONS_CAST (widget);
  GtkRoot *root;
  GdkSurface *surface;
...
static void
nyx_app_window_state_buttons_dispose (GObject *object)
{
  NyxAppWindowStateButtons *self = NYX_APP_WINDOW_STATE_BUTTONS_CAST (object);

  _clear_stored_settings (self);

  gtk_widget_dispose_template (GTK_WIDGET (object), NYX_APP_TYPE_WINDOW_STATE_BUTTONS);

  G_OBJECT_CLASS (parent_class)->dispose (object);
}

static void
nyx_app_window_state_buttons_set_property (GObject *object, guint prop_id,
...
static void
nyx_app_window_state_buttons_class_init (NyxAppWindowStateButtonsClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

  GST_DEBUG_CATEGORY_INIT (GST_CAT_DEFAULT, "nyxappwindowstatebuttons", 0,
      "Nyx App Window State Buttons");

  gobject_class->constructed = nyx_app_window_state_buttons_constructed;
  gobject_class->set_property = nyx_app_window_state_buttons_set_property;
  gobject_class->dispose = nyx_app_window_state_buttons_dispose;

  widget_class->realize = nyx_app_window_state_buttons_realize;
  widget_class->unrealize = nyx_app_window_state_buttons_unrealize;
  ...
  gtk_widget_class_set_template_from_resource (widget_class,
      NYX_APP_RESOURCE_PREFIX "/ui/nyx-app-window-state-buttons.ui");

  gtk_widget_class_bind_template_child (widget_class, NyxAppWindowStateButtons, minimize_button);
  gtk_widget_class_bind_template_child (widget_class, NyxAppWindowStateButtons, maximize_button);
  gtk_widget_class_bind_template_child (widget_class, NyxAppWindowStateButtons, close_button);
  ...
  gtk_widget_class_set_css_name (widget_class, "nyx-app-window-state-buttons");
}
